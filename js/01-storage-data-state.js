...
window.BorionSecureVault = window.SecureJsonVault.forApp({
  appId: 'borion-finance',
  appName: 'Borion Finance',
  dialogTheme: 'borion',
  driveTrustMode: true,
  isSensitive: value => !!(
    value &&
    ...
    ((value.dataByProfile && typeof value.dataByProfile === 'object') ||
      (value.pendingByProfile && typeof value.pendingByProfile === 'object'))
  ) || !!(
    value &&
    typeof value === 'object' &&
    value.type === 'multicap-profile-backup' &&
    value.profile &&
    value.data
  )
});
/* V7.8.2 — O cofre da INTEGRAÇÃO passa a operar em modo Drive confiável, igual
   ao cofre principal. Motivo real: o bridge e o ack são escritos por um app e
   lidos por OUTRO (Amanda Estética / Marco Iris <-> Borion). Cada aplicativo
   cifrava esses arquivos com a chave do próprio cofre, então o Borion abria o
   arquivo do app de origem, não conseguia decifrar e caía num pedido de "senha
   mestra" que nunca abriria nada. Era exatamente esse o resquício que ainda
   aparecia numa conta e não em outra: só pedia na conta que tinha uma
   integração conectada. A partir daqui o canal de integração é JSON normal,
   dentro de uma pasta já protegida pela conta Google e pelo escopo drive.file.
   A base financeira (current.json) continua no mesmo cofre de sempre. */
window.BorionIntegrationVault = window.SecureJsonVault.forApp({
  ...
  isSensitive: value => !!(
    value &&
    typeof value === 'object' &&
    (value.schema === 'borion.interop.snapshot' || value.schema === 'borion.interop.ack')
  )
});
...
function getStorageMode(){ return 'google_drive'; }
function setStorageMode(){ writeJSON(LS_STORAGE_MODE, 'google_drive'); }
...
/* V6.46.47 — recuperação temporária de alteração ainda não confirmada no Drive.
   O navegador não consegue continuar executando JavaScript depois que a aba/PWA é
   encerrada. Para não perder uma inclusão feita imediatamente antes de sair, o Borion
   mantém UMA cópia local temporária somente enquanto o Google Drive ainda não confirmou
   o snapshot. Assim que a confirmação real ocorre, essa cópia é apagada. */
const BORION_STRICT_RECOVERY_PREFIX='borion_strict_pending_recovery_v64647_';
const BORION_STRICT_RECOVERY_INDEX='borion_strict_pending_recovery_index_v64647';
const BorionStrictPendingRecovery={
  _clone(value){return BorionStrictDrive._clone(value);},
  _inflight:new Map(),
  _folderId(){
    try{return String((window.GoogleDriveProvider&&GoogleDriveProvider.folderId)||'');}
    catch(e){return '';}
  },
  _key(folderId){return BORION_STRICT_RECOVERY_PREFIX+encodeURIComponent(String(folderId||''));},
  persist(reason='data_change'){
    this.clear();
    return Promise.resolve({saved:false,reason:'handled_by_encrypted_fast_wal'});
  },
  async read(folderId){
    this.clear(folderId);
    return null;
  ...
  toAccountPayload(recovery,remoteSnapshot){
    if(!recovery||!remoteSnapshot)return null;
    const local=this._clone(remoteSnapshot);
    const profileMap=new Map((local.profiles||[]).filter(Boolean).map(p=>[String(p.id),p]));
    (recovery.profiles||[]).filter(Boolean).forEach(p=>profileMap.set(String(p.id),this._clone(p)));
    local.profiles=Array.from(profileMap.values());
    local.dataByProfile=Object.assign({},local.dataByProfile||{});
    Object.entries(recovery.dataByProfile||{}).forEach(([id,data])=>{local.dataByProfile[String(id)]=this._clone(data);});
    local.config=Object.assign({},local.config||{},this._clone(recovery.config||{}));
    local.__syncMeta640=Object.assign({},local.__syncMeta640||{});
    local.__syncMeta640.profileTombstones=Object.assign({},local.__syncMeta640.profileTombstones||{},this._clone(recovery.profileTombstones||{}));
    local.recoveryMeta={source:'temporary_exit_recovery',savedAt:recovery.savedAt||Date.now(),operationId:recovery.operationId||null};
    return local;
  },
  clear(folderId){
    const id=String(folderId||this._folderId()||'');
    if(!id)return false;
    try{
      localStorage.removeItem(this._key(id));
      if(localStorage.getItem(BORION_STRICT_RECOVERY_INDEX)===id)localStorage.removeItem(BORION_STRICT_RECOVERY_INDEX);
      return true;
    }catch(e){return false;}
  }
};
window.BorionStrictPendingRecovery=BorionStrictPendingRecovery;
...
const DEFAULT_ICON_COLORS = { liquidez:'#22c55e', bens:'#3b6bf0', investimentos:'#cca160', moedas:'#38bdf8', dividas:'#ef4444', receita:'#22c55e', despesas:'#ef4444', investir:'#3b6bf0', saldo:'#eef1f4' };
const ICON_COLOR_LABELS = { liquidez:'Saldo em Contas', bens:'Bens', investimentos:'Investimentos', moedas:'Moedas estrangeiras', dividas:'Dívidas', receita:'Receita', despesas:'Despesas', investir:'Investir', saldo:'Saldo' };
const FONT_STACKS = {
...
const DEFAULT_MODULES = { reserves:true, imports:true, investments:true, agenda:true, notes:true, patrimony:true, charts:true };
const DEFAULT_DASHBOARD_WIDGETS = ['fluxoMensal','evolucaoPatrimonio','evolucaoDividasCartao','distribuicaoPatrimonio','gastosCategoria','gastosCartao','distribuicaoBanco','resumoBanco','evolucaoMoedas'];
...
function emptyData(){
  return {
    ...
    investirPlanejado: {},
    transacoes: [],
    /* V7.7.0 — registros auditáveis de reembolso/estorno. A transação vinculada
       mantém a exibição em Recebimentos; este array guarda o destino e os vínculos
       com conta, fatura, compra parcelada ou crédito da loja. */
    refunds: [],
    fixas: [],
    ...
    moedasEstrangeiras:{positions:[],moves:[],rateHistory:[]},
    contas: [],
    cartoes: [],
    boletos: [],
    transferencias: [],
    ...
    metas: [],
    notificacoes: [],
    notes: {items:[], activeId:null},
    /* V7.9.0 — Estúdio de Gráficos: cada gráfico montado pelo usuário vive aqui e
       viaja no current.json, então o painel acompanha o perfil em qualquer dispositivo. */
    chartsStudio: {version:1, charts:[]},
    patrimonioHistorico: {},
    ...
    importBatches: [],
    uiPreferences:{budgetDateSort:{receita:'desc',fixa:'desc',variavel:'desc',transferencias:'desc'},reserveViewMode:'columns',floatingNotes:{enabled:false,text:'',minimized:true,x:null,y:null},borionCalculator:{enabled:false,minimized:true,side:'right',y:null,panelW:390,panelH:650,mode:'standard',history:[],memory:0,angle:'DEG',converter:{category:'currency',amount:'1',from:'BRL',to:'USD',rates:{},units:{length:['m','km'],mass:['kg','g'],temperature:['C','F']}}}}
  };
}
function migrateData(d, migrationContext={}){
  ...
  if(!d.fixas) d.fixas=[];
  if(!Array.isArray(d.refunds)) d.refunds=[];
  d.refunds=d.refunds.filter(r=>r&&typeof r==='object').map((r,i)=>{
    if(!r.id)r.id=migrationUid6401('refunds',{transactionId:r.transactionId||'',mode:r.mode||'account',date:r.date||'',amount:r.amount||r.value||0},i);
    if(!['account','invoice','purchase','store_credit'].includes(r.mode))r.mode='account';
    r.amount=Math.round((Number(r.amount!=null?r.amount:r.value)||0)*100)/100;
    if(!r.allocations||typeof r.allocations!=='object'||Array.isArray(r.allocations))r.allocations={};
    if(r.reducesExpenses==null)r.reducesExpenses=true;
    return r;
  });
  ...
  if(!d.metas) d.metas=[];
  /* V7.7.6 — moedas estrangeiras são posições patrimoniais em moeda original.
     A cotação é manual e os movimentos ficam separados de Receita/Despesa. */
  if(!d.moedasEstrangeiras || typeof d.moedasEstrangeiras!=='object' || Array.isArray(d.moedasEstrangeiras)) d.moedasEstrangeiras={positions:[],moves:[],rateHistory:[]};
  if(!Array.isArray(d.moedasEstrangeiras.positions)) d.moedasEstrangeiras.positions=[];
  if(!Array.isArray(d.moedasEstrangeiras.moves)) d.moedasEstrangeiras.moves=[];
  if(!Array.isArray(d.moedasEstrangeiras.rateHistory)) d.moedasEstrangeiras.rateHistory=[];
  d.moedasEstrangeiras.positions=d.moedasEstrangeiras.positions.filter(p=>p&&typeof p==='object').map((p,i)=>{
    if(!p.id)p.id=migrationUid6401('moedasEstrangeiras.positions',{currency:p.currency||'USD',location:p.location||'',createdAt:p.createdAt||0},i);
    p.currency=String(p.currency||'USD').toUpperCase();
    p.holdingType=p.holdingType==='account'?'account':'cash';
    p.location=String(p.location||'Não informado');
    p.amount=Math.max(0,Math.round((Number(p.amount)||0)*10000)/10000);
    p.currentRate=Math.max(0,Math.round((Number(p.currentRate)||0)*10000)/10000);
    p.averageRate=Math.max(0,Math.round((Number(p.averageRate)||p.currentRate||0)*10000)/10000);
    if(!p.createdAt)p.createdAt=0;
    if(!p.updatedAt)p.updatedAt=p.createdAt;
    return p;
  });
  const _foreignPositionIds=new Set(d.moedasEstrangeiras.positions.map(p=>String(p.id)));
  d.moedasEstrangeiras.moves=d.moedasEstrangeiras.moves.filter(m=>m&&typeof m==='object').map((m,i)=>{
    if(!m.id)m.id=migrationUid6401('moedasEstrangeiras.moves',{positionId:m.positionId||'',type:m.type||'opening',date:m.date||'',foreignAmount:m.foreignAmount||0},i);
    m.type=String(m.type||'opening');
    m.foreignAmount=Math.round((Number(m.foreignAmount)||0)*10000)/10000;
    m.rate=Math.max(0,Math.round((Number(m.rate)||0)*10000)/10000);
    m.brlAmount=Math.max(0,Math.round((Number(m.brlAmount)||0)*100)/100);
    if(!m.createdAt)m.createdAt=0;
    if(m.positionId!=null)m.positionId=String(m.positionId);
    if(m.destinationPositionId!=null)m.destinationPositionId=String(m.destinationPositionId);
    if(m.positionId&&!_foreignPositionIds.has(m.positionId))m.orphanedPosition=true;
    return m;
  });
  d.moedasEstrangeiras.rateHistory=d.moedasEstrangeiras.rateHistory.filter(r=>r&&typeof r==='object').map((r,i)=>{
    if(!r.id)r.id=migrationUid6401('moedasEstrangeiras.rateHistory',{currency:r.currency||'USD',date:r.date||'',rate:r.rate||0},i);
    r.currency=String(r.currency||'USD').toUpperCase();
    r.date=String(r.date||todayISO());
    r.rate=Math.max(0,Math.round((Number(r.rate)||0)*10000)/10000);
    r.source=String(r.source||'manual');
    r.providerDate=String(r.providerDate||'');
    r.fetchedAt=Number(r.fetchedAt)||0;
    return r;
  }).filter(r=>r.rate>0);
  if(!d.notificacoes) d.notificacoes=[];
  /* V7.9.0 — Estúdio de Gráficos. Perfis antigos nascem com o painel vazio (o
     usuário monta os próprios gráficos); nada é inventado aqui. A normalização
     só garante que a estrutura exista e que cada gráfico tenha id. */
  if(!d.chartsStudio || typeof d.chartsStudio!=='object' || Array.isArray(d.chartsStudio)) d.chartsStudio={version:1, charts:[]};
  if(!Array.isArray(d.chartsStudio.charts)) d.chartsStudio.charts=[];
  d.chartsStudio.charts=d.chartsStudio.charts.filter(c=>c&&typeof c==='object').map((c,i)=>Object.assign({},c,{
    id:String(c.id||migrationUid6401('chartsStudio.charts',{titulo:c.titulo||'',fonte:c.fonte||'',visual:c.visual||''},i))
  }));
  if(!d.notes || typeof d.notes!=='object' || Array.isArray(d.notes)) d.notes={items:[],activeId:null};
  if(!Array.isArray(d.notes.items)) d.notes.items=[];
  d.notes.items=d.notes.items.filter(n=>n&&typeof n==='object').map((n,i)=>({id:String(n.id||migrationUid6401('notes.items',{title:n.title||'Nota',createdAt:n.createdAt||0},i)),title:String(n.title||'Nota').slice(0,120),content:typeof n.content==='string'?n.content:'',createdAt:Number(n.createdAt)||Date.now(),updatedAt:Number(n.updatedAt)||Number(n.createdAt)||Date.now()}));
  if(d.notes.activeId==null || !d.notes.items.some(n=>String(n.id)===String(d.notes.activeId))) d.notes.activeId=d.notes.items[0]?d.notes.items[0].id:null;
  ...
  // V7.5.2 — remove somente preferências do módulo Análise, sem tocar em qualquer dado financeiro.
  if(d.modules && Object.prototype.hasOwnProperty.call(d.modules,'analytics')) delete d.modules.analytics;
  if(d.uiPreferences.startupView==='analise') d.uiPreferences.startupView='overview';
  if(Object.prototype.hasOwnProperty.call(d.uiPreferences,'analiseAvancada')) delete d.uiPreferences.analiseAvancada;
  if(Object.prototype.hasOwnProperty.call(d.uiPreferences,'analyticsDashboardV2Introduced')) delete d.uiPreferences.analyticsDashboardV2Introduced;
  if(d.uiPreferences.orderPreferences && d.uiPreferences.orderPreferences.orders && Array.isArray(d.uiPreferences.orderPreferences.orders.modules)) d.uiPreferences.orderPreferences.orders.modules=d.uiPreferences.orderPreferences.orders.modules.filter(id=>id!=='analise');
  // V7.4.0 — o Assistente de IA foi removido do produto; apaga preferências e histórico antigos.
  if(Object.prototype.hasOwnProperty.call(d.uiPreferences,'borionAI')) delete d.uiPreferences.borionAI;
  d.uiPreferences.reserveViewMode=d.uiPreferences.reserveViewMode==='list'?'list':'columns';
  ...
  if(!d.uiPreferences.borionCalculator || typeof d.uiPreferences.borionCalculator!=='object' || Array.isArray(d.uiPreferences.borionCalculator)) d.uiPreferences.borionCalculator={enabled:false,minimized:true,side:'right',y:null,panelW:390,panelH:650,mode:'standard',history:[],memory:0,angle:'DEG',converter:{category:'currency',amount:'1',from:'BRL',to:'USD',rates:{},units:{length:['m','km'],mass:['kg','g'],temperature:['C','F']}}};
  if(typeof d.uiPreferences.borionCalculator.enabled!=='boolean') d.uiPreferences.borionCalculator.enabled=false;
  if(typeof d.uiPreferences.borionCalculator.minimized!=='boolean') d.uiPreferences.borionCalculator.minimized=true;
  if(!Array.isArray(d.uiPreferences.borionCalculator.history)) d.uiPreferences.borionCalculator.history=[];
  if(d.uiPreferences.borionCalculator.history.length>60) d.uiPreferences.borionCalculator.history=d.uiPreferences.borionCalculator.history.slice(0,60);
  ...
  (d.cartoes||[]).forEach(c=>{
    ...
    (c.parcelas||[]).forEach(p=>{
      ...
      /* V6.46.64 — o gerador de despesas espelhadas do cartão interpretava status
         ausente como Pago e podia criar todas as competências futuras já quitadas.
         Detectamos o padrão suspeito em bloco: somente parcelas variáveis, somente meses
         futuros, sem fatura paga, sem baixa individual e sem marcador manual. Pagamentos
         reais continuam preservados. */
      if(p.despesaTipo==='variavel'){
        const _currentYM=String(todayISO()).slice(0,7);
        const _futureLinked=(d.transacoes||[]).filter(tx=>tx&&tx.tipo==='variavel'&&tx.viaParcelaId===p.id&&String(tx.data||'').slice(0,7)>_currentYM);
        const _repairable=_futureLinked.filter(tx=>{
          const _txYM=String(tx.data||'').slice(0,7);
          const _invoicePaid=(c.faturasPagas||[]).some(pg=>pg&&pg.competencia===_txYM);
          const _individualPaid=(p.pagamentosIndividuais||[]).some(pg=>pg&&pg.competencia===_txYM&&pg.pago!==false);
          const _manual=!!tx.statusPagamentoManualAt||tx.statusPagamentoOrigem==='manual';
          return !_invoicePaid&&!_individualPaid&&!_manual;
        });
        const _looksAutoPaid=p.statusPagamento!=='Pago'&&_repairable.length>0&&_repairable.every(tx=>tx.statusPagamento==='Pago');
        if(_looksAutoPaid)_repairable.forEach(tx=>{
          tx.statusPagamento='Em aberto';
          tx.statusPagamentoOrigem='reparado_v6.46.64';
        });
      }
...
/* V6.46.67 — uma conta recém-inicializada pelo próprio Borion é um estado válido:
   current.json existe, mas ainda não há perfil até a pessoa criar ou importar um.
   A V6.46.67 criava exatamente esse arquivo e depois o rejeitava no próximo boot,
   causando a mensagem falsa de base inválida. Só aceitamos vazio quando todos os
   marcadores de inicialização conferem; backups comuns vazios continuam bloqueados. */
function isValidInitialEmptyBorionAccount(obj){
  if(!obj || (obj.type!=='borion-account-backup' && obj.type!=='multicap-full-backup')) return false;
  if(obj.backupType!=='initial') return false;
  if(!Array.isArray(obj.profiles) || obj.profiles.length!==0) return false;
  if(!obj.dataByProfile || typeof obj.dataByProfile!=='object' || Array.isArray(obj.dataByProfile)) return false;
  if(Object.keys(obj.dataByProfile).length!==0) return false;
  return Number(obj.profileCount||0)===0;
}
function validateBorionJson(obj){
  ...
  if(obj.type==='borion-account-backup' || obj.type==='multicap-full-backup'){
    const validInitialEmpty=isValidInitialEmptyBorionAccount(obj);
    if(!Array.isArray(obj.profiles) || (!obj.profiles.length && !validInitialEmpty)) errors.push('Backup completo sem nenhum perfil dentro de "profiles".');
...
/* V6.25.1 — O filtro superior representa instituições financeiras, não registros.
   Portanto: unifica nomes como Nubank/NUBANK.
   V7.1.4 — a Carteira (dinheiro físico) passou a entrar também, a pedido do Pedro,
   pra dar pra filtrar só os lançamentos da carteira igual já dava com bancos/contas. */
function bankFilterNames(){
  ...
  activeAccounts().forEach(c=>add(c.nome));
...
/* ---------------- Liquidez: ajuste de saldo por banco (usado por boleto pago,
...
const S = {
  ...
  month: todayYM(),
  filters: {
    receita:{busca:'',categorias:[],origens:[],dataDe:'',dataAte:'',dateSort:'asc'},
    fixa:{busca:'',categorias:[],origens:[],dataDe:'',dataAte:'',dateSort:'asc'},
    variavel:{busca:'',categorias:[],origens:[],dataDe:'',dataAte:'',dateSort:'asc'}
  },
...
function saveCurrentData(options={}){
  if(S.currentProfile && S.data){
    ...
    if(window.BorionStrictPendingRecovery&&window.BorionStrictDrive&&BorionStrictDrive.shouldUseMemory()){
      BorionStrictPendingRecovery.persist(options.finalConfirmation?'final_confirmation':'data_change');
    }
    if(window.CloudStorage && CloudStorage.user){ CloudStorage.queueSave(S.currentProfile.id, S.data); }
    // v7.0 SECURITY — antes de qualquer chamada de rede, preserva somente o
    // perfil alterado em um WAL durável no IndexedDB e dispara um checkpoint
    // isolado. As correções funcionais da v6.46.67 permanecem intactas.
    try{
      const _drive=window.GoogleDriveProvider||null;
      if(_drive&&_drive.isConnected&&_drive.isConnected()&&!_drive._queueOperationId&&window.BorionSyncCore&&BorionSyncCore.uuid640){
        _drive._queueOperationId=BorionSyncCore.uuid640();
      }
      if(window.BorionFastWal700&&typeof BorionFastWal700.persistCurrent==='function'){
        BorionFastWal700.persistCurrent({
          reason:options.finalConfirmation?'final_confirmation':'data_change',
          operationId:_drive&&_drive._queueOperationId||null
        });
      }
      if(window.BorionSecurity700&&typeof BorionSecurity700.captureCurrent==='function'){
        BorionSecurity700.captureCurrent(options.finalConfirmation?'final_confirmation':'data_change');
      }
    }catch(e){console.warn('[BORION][FAST_SAVE][CAPTURE_WARN]',e);}
