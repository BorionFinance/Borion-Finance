...
/* V6.46.67 — uma queda é legítima quando cada registro removido possui um
   tombstone NOVO, criado nesta mesma leva de alterações. Não basta existir um
   tombstone antigo: o ID precisava estar na base anterior, estar ausente na
   próxima base e não possuir tombstone na base anterior. Essa checagem mantém
   o bloqueio contra base vazia/corrompida e libera somente exclusões rastreáveis. */
function borionPathFromKey(key){
  return BORION_CRITICAL_PATHS.find(path=>borionPathKey(path)===key) || null;
}
function borionRecordIds(data,path){
  return new Set(borionReadPath(data,path).filter(item=>item&&item.id!=null).map(item=>String(item.id)));
}
function borionTombstonesAt(data,key){
  const value=data&&data.__syncMeta&&data.__syncMeta.tombstones&&data.__syncMeta.tombstones[key];
  return value&&typeof value==='object'&&!Array.isArray(value)?value:{};
}
function borionNewDeletionCoverage(nextPayload,baselinePayload,key){
  const path=borionPathFromKey(key);
  if(!path||!nextPayload||!baselinePayload)return 0;
  const nextByProfile=nextPayload.dataByProfile||{};
  const baseByProfile=baselinePayload.dataByProfile||{};
  let covered=0;
  Object.keys(baseByProfile).forEach(profileId=>{
    const beforeData=baseByProfile[profileId]||{};
    const afterData=nextByProfile[profileId]||{};
    const beforeIds=borionRecordIds(beforeData,path);
    const afterIds=borionRecordIds(afterData,path);
    const beforeTombstones=borionTombstonesAt(beforeData,key);
    const afterTombstones=borionTombstonesAt(afterData,key);
    beforeIds.forEach(id=>{
      if(afterIds.has(id))return;
      if(!Object.prototype.hasOwnProperty.call(afterTombstones,id))return;
      if(Object.prototype.hasOwnProperty.call(beforeTombstones,id))return;
      covered+=1;
    });
  });
  return covered;
}
function isSuspiciousDropCoveredByTombstones(nextCounts,baselineCounts,nextPayload,baselinePayload,reasons){
  const list=Array.isArray(reasons)?reasons:[];
  if(!list.length||!nextPayload||!baselinePayload)return false;
  const calculatedNext=countAccountRecords(nextPayload);
  const calculatedBefore=countAccountRecords(baselinePayload);
  return list.every(reason=>{
    if(!reason||reason.key==='__profiles')return false;
    const required=Math.max(0,(Number(reason.before)||0)-(Number(reason.after)||0));
    if(required<=0)return false;
    // A autorização só vale se os payloads usados para conferir os IDs forem
    // exatamente os mesmos números usados pelo Data Guard para acusar a queda.
    if((Number(calculatedBefore[reason.key])||0)!==(Number(baselineCounts?.[reason.key])||0))return false;
    if((Number(calculatedNext[reason.key])||0)!==(Number(nextCounts?.[reason.key])||0))return false;
    return borionNewDeletionCoverage(nextPayload,baselinePayload,reason.key)>=required;
  });
}
...
window.BorionDataGuard = {
  ...
  newDeletionCoverage: borionNewDeletionCoverage,
  isSuspiciousDropCoveredByTombstones,
