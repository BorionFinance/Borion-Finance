...
const BORION_SYNCABLE_COLLECTIONS = [
  'transacoes','refunds','fixas','fixaPagamentos','estornos','liquidez','bens','contas',
  ...
  ['moedasEstrangeiras','positions'], ['moedasEstrangeiras','moves'], ['moedasEstrangeiras','rateHistory'],
  ['reservas','boxes'], ['reservas','moves'], ['reservas','monthlyReports'],
  ['cheques','items'], ['notes','items']
];
...
function mergeCollection(collectionKey,baseArr,localArr,remoteArr,baseTombstones,localTombstones,remoteTombstones){
  ...
  for(const id of allIds){
    ...
    if(tomb){
      const surviving=l||r;
      // v7.0 SECURITY — exclusão explícita sempre vence no estado ativo.
      // Uma edição concorrente fica preservada na quarentena de conflitos para
      // possível recuperação manual, mas nunca recoloca o item na coleção.
      const survivorChanged=!!(surviving&&b&&!sameValue(surviving,b));
      if(survivorChanged&&(tl||tr)){
        conflicts.push({collection:collectionKey,id,kind:'edit_vs_delete_quarantined',resolution:'tombstone_wins',base:safeClone(b||null),local:tl?null:safeClone(l||null),remote:tr?null:safeClone(r||null),tombstone:safeClone(tomb)});
      }
      mergedTombstones[id]=safeClone(tomb);
      continue;
    }
...
function mergeProfileMetadata(base,local,remote,id,conflicts){
  // v7.0 SECURITY — revisões por campo impedem que ausência, fallback ou estado
  // transitório substituam silenciosamente nome, foto, cor ou senha válidos.
  if(typeof window!=='undefined'&&window.BorionAccountIntegrity700&&typeof BorionAccountIntegrity700.mergeProfileMetadata==='function'){
    return BorionAccountIntegrity700.mergeProfileMetadata(base,local,remote,id,conflicts);
  }
  const localConf=[];
