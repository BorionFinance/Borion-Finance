...
const borionCloudOnlyStores = new Map(BORION_IDB640_STORES.map(name=>[name,new Map()]));

function cloneCloudOnlyRecord(value){
  if(value==null)return value;
  try{return structuredClone(value);}catch(_){return JSON.parse(JSON.stringify(value));}
}
function cloudOnlyStore(name){
  const store=borionCloudOnlyStores.get(name);
  if(!store)throw new Error('Armazenamento temporario desconhecido: '+name);
  return store;
}
async function borionIdb640Open(){return {cloudOnly:true,close(){}};}
async function borionIdb640Put(store, record){
  if(!record||record.id==null)throw new Error('Registro temporario sem ID.');
  cloudOnlyStore(store).set(String(record.id),cloneCloudOnlyRecord(record));
  return record;
}
async function borionIdb640Delete(store, id){
  cloudOnlyStore(store).delete(String(id));
  return true;
}
async function borionIdb640Get(store, id){
  return cloneCloudOnlyRecord(cloudOnlyStore(store).get(String(id))||null);
}
async function borionIdb640GetAll(store){
  return Array.from(cloudOnlyStore(store).values()).map(cloneCloudOnlyRecord);
}
try{if(globalThis.indexedDB)indexedDB.deleteDatabase(BORION_IDB640_NAME);}catch(_){}
