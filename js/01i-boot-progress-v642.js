...
(function(global){
  ...
  const BootProgress={
    ...
    fail(error,options={}){this._failed=true;if(this.currentStage)BorionPerf.endStage('boot_'+this.currentStage,{failed:true});this._clearTimers();BorionPerf.finish({status:'failed',failedStage:this.currentStage,errorCode:error&&error.code||null});this._retryHandler=options.retry||this._retryHandler;this._reconnectHandler=options.reconnect||this._reconnectHandler;this._backHandler=options.back||this._backHandler;const title=document.getElementById('borion-boot-title'),detail=document.getElementById('borion-boot-detail'),spinner=document.querySelector('.borion-boot-spinner'),errorBox=document.getElementById('borion-boot-error');if(title)title.textContent='Não foi possível concluir a abertura';if(detail)detail.textContent=this._friendlyError(error);if(spinner)spinner.hidden=true;if(errorBox){const stageLabel=(this.stages[this.currentStage]&&this.stages[this.currentStage].label)||this.currentStage||'Inicialização';const technical=safeText((error&&((error.code?error.code+': ':'')+(error.message||'')))||'Falha desconhecida');errorBox.hidden=false;errorBox.innerHTML=`<div class="borion-boot-error-stage">Etapa: ${safeText(stageLabel)}</div><div class="borion-boot-actions"><button type="button" id="borion-boot-retry">Tentar novamente</button>${this._isAuthError(error)?'<button type="button" id="borion-boot-reconnect">Reconectar conta Google</button>':''}<button type="button" id="borion-boot-details">Ver detalhes técnicos</button>${options.allowBack!==false?'<button type="button" class="secondary" id="borion-boot-back">Voltar ao login</button>':''}</div><pre id="borion-boot-technical" hidden>${technical}</pre>`;const retry=document.getElementById('borion-boot-retry');if(retry)retry.onclick=()=>this.retry();const reconnect=document.getElementById('borion-boot-reconnect');if(reconnect)reconnect.onclick=()=>this.reconnect();const details=document.getElementById('borion-boot-details');if(details)details.onclick=()=>{const pre=document.getElementById('borion-boot-technical');if(pre)pre.hidden=!pre.hidden;};const back=document.getElementById('borion-boot-back');if(back)back.onclick=()=>this.back();}return this;},
    ...
    back(){if(typeof this._backHandler==='function')return this._backHandler();if(global.CloudAuth&&typeof CloudAuth.render==='function')CloudAuth.render();else location.reload();}
  };
