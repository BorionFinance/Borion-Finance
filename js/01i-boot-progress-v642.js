...
(function(global){
  ...
  const BootProgress={
    ...
    fail(error,options={}){this._failed=true;if(this.currentStage)BorionPerf.endStage('boot_'+this.currentStage,{failed:true});this._clearTimers();BorionPerf.finish({status:'failed',failedStage:this.currentStage,errorCode:error&&error.code||null});this._retryHandler=options.retry||this._retryHandler;this._reconnectHandler=options.reconnect||this._reconnectHandler;this._backHandler=options.back||this._backHandler;const title=document.getElementById('borion-boot-title'),detail=document.getElementById('borion-boot-detail'),spinner=document.querySelector('.borion-boot-spinner'),errorBox=document.getElementById('borion-boot-error');const auth=this._isAuthError(error),friendly=this._friendlyError(error);if(global.console&&console.error)console.error('[BORION][BOOT] abertura interrompida:',error);if(title)title.textContent='Não foi possível concluir a abertura';if(detail)detail.textContent=friendly+' Você pode tentar novamente sem apagar a pasta vinculada.';if(spinner)spinner.hidden=true;if(errorBox){errorBox.hidden=false;const primary=auth?'Continuar com Google':'Tentar novamente';errorBox.innerHTML=`<div class="borion-boot-actions"><button type="button" id="borion-boot-retry" class="borion-boot-action-btn">${primary}</button><button type="button" id="borion-boot-back" class="borion-boot-action-btn">Trocar conta Google</button></div>`;const retry=document.getElementById('borion-boot-retry'),back=document.getElementById('borion-boot-back');if(retry)retry.onclick=()=>auth?this.reconnect():this.retry();if(back)back.onclick=()=>this.back();}return this;},
