...
const FS_ACCESS_SUPPORTED = typeof window!=='undefined' && window.isSecureContext===true && typeof window.showDirectoryPicker==='function';
const IDB_NAME = 'borion_handles', IDB_STORE = 'handles';
const BORION_APP_VERSION = (window.BORION_RELEASE&&BORION_RELEASE.label)||'7.0';
...
function idbOpen(){
  return new Promise((resolve,reject)=>{
    ...
    req.onerror=()=>reject(req.error||new Error('Não foi possível abrir o armazenamento da pasta local.'));
    req.onblocked=()=>reject(new Error('O armazenamento da pasta local está bloqueado por outra aba.'));
  });
}
async function idbGet(key){
  const db=await idbOpen();
  try{return await new Promise((resolve,reject)=>{const tx=db.transaction(IDB_STORE,'readonly');const req=tx.objectStore(IDB_STORE).get(key);req.onsuccess=()=>resolve(req.result||null);req.onerror=()=>reject(req.error);});}
  finally{db.close();}
}
async function idbSet(key,val){
  const db=await idbOpen();
  try{return await new Promise((resolve,reject)=>{const tx=db.transaction(IDB_STORE,'readwrite');tx.objectStore(IDB_STORE).put(val,key);tx.oncomplete=()=>resolve(true);tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error||new Error('Gravação da pasta local cancelada.'));});}
  finally{db.close();}
}
async function idbDel(key){
  const db=await idbOpen();
  try{return await new Promise((resolve,reject)=>{const tx=db.transaction(IDB_STORE,'readwrite');tx.objectStore(IDB_STORE).delete(key);tx.oncomplete=()=>resolve(true);tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error||new Error('Remoção da pasta local cancelada.'));});}
  finally{db.close();}
}
...
function normalizeProfileForBackup(p){
  if(!p||p.id==null) throw Object.assign(new Error('Perfil inválido no backup.'),{code:'PROFILE_ID_MISSING'});
  const name=typeof p.name==='string'?p.name.trim():'';
  if(!name) throw Object.assign(new Error('O perfil '+String(p.id)+' está sem nome; o backup foi bloqueado para não salvar o fallback “Perfil”.'),{code:'PROFILE_METADATA_INCOMPLETE',profileId:String(p.id)});
  const out={id:p.id,name,email:p.email==null?'':String(p.email),cloud:!!p.cloud};
  if(Object.prototype.hasOwnProperty.call(p,'avatarColor'))out.avatarColor=p.avatarColor;
  else if(Object.prototype.hasOwnProperty.call(p,'avatar_color'))out.avatarColor=p.avatar_color;
  if(Object.prototype.hasOwnProperty.call(p,'avatarImage'))out.avatarImage=p.avatarImage;
  else if(Object.prototype.hasOwnProperty.call(p,'avatar_image'))out.avatarImage=p.avatar_image;
  if(Object.prototype.hasOwnProperty.call(p,'passwordHash'))out.passwordHash=p.passwordHash;
  else if(Object.prototype.hasOwnProperty.call(p,'password_hash'))out.passwordHash=p.password_hash;
  if(Object.prototype.hasOwnProperty.call(p,'salt'))out.salt=p.salt;
  else if(Object.prototype.hasOwnProperty.call(p,'password_salt'))out.salt=p.password_salt;
  out.createdAt=p.createdAt||p.created_at||null;out.updatedAt=p.updatedAt||p.updated_at||null;
  if(p.__fieldRevisions&&typeof p.__fieldRevisions==='object')out.__fieldRevisions=backupSafeClone(p.__fieldRevisions);
  return out;
}
...
async function buildLocalAccountBackupPayload(backupType='manual', reason=''){
  ...
  const dataByProfile = {};
  if(!profiles.length) throw Object.assign(new Error('A conta ainda não possui perfis carregados; o snapshot vazio foi bloqueado.'),{code:'CURRENT_JSON_PARTIAL'});
  for(const p of profiles){
    let d = (S.currentProfile && p.id===S.currentProfile.id && S.data) ? S.data : getProfileData(p.id);
    if(d==null && typeof idbGetProfileData==='function') d = await idbGetProfileData(p.id);
    if(d==null) throw Object.assign(new Error('Os dados do perfil '+String(p.id)+' não terminaram de carregar; o snapshot remoto não foi alterado.'),{code:'PROFILE_DATA_MISSING',profileId:String(p.id)});
    dataByProfile[p.id] = migrateData(d, {profileId:p.id});
  }
  const accountSyncMeta={schemaVersion:(window.BorionSyncCore&&BorionSyncCore.BORION_DATA_SCHEMA_VERSION)||6401,profileTombstones:(typeof getProfileTombstones6401==='function'?getProfileTombstones6401():{}),accountEverHadProfiles:profiles.length>0,profileCountHighWater:profiles.length};
  const compact = {profiles, dataByProfile, config:S.config||{}, __syncMeta640:accountSyncMeta};
  const hash = window.BorionSyncCore ? await BorionSyncCore.checksumOf(compact) : (typeof sha256Hex==='function' ? await sha256Hex(JSON.stringify(compact)) : '');
  return {
  ...
  try{
    ...
    for(const p of profiles){
      if(!dataByProfile[p.id]){
        ...
        if(local==null && typeof idbGetProfileData==='function') local = await idbGetProfileData(p.id);
        if(local==null) throw Object.assign(new Error('O perfil '+String(p.id)+' existe, mas seus dados não foram encontrados; snapshot bloqueado.'),{code:'PROFILE_DATA_MISSING',profileId:String(p.id)});
        dataByProfile[p.id]=migrateData(local, {profileId:p.id});
      }
    }
    const accountSyncMeta={schemaVersion:(window.BorionSyncCore&&BorionSyncCore.BORION_DATA_SCHEMA_VERSION)||6401,profileTombstones:(typeof getProfileTombstones6401==='function'?getProfileTombstones6401():{}),accountEverHadProfiles:profiles.length>0,profileCountHighWater:profiles.length};
    const compact={profiles,dataByProfile,config:S.config||{},__syncMeta640:accountSyncMeta};
...
async function finalizeBackupSnapshot(payload, backupType='manual', reason=''){
  ...
  const checksum=window.BorionSyncCore?await BorionSyncCore.checksumOf(canonical):(typeof sha256Hex==='function'?await sha256Hex(JSON.stringify(canonical)):((out.integrity&&out.integrity.sha256)||''));
  out.snapshotChecksum=checksum;
  out.integrity=Object.assign({},out.integrity||{},{snapshotSha256:checksum});
  return out;
}
/* V7.9.4 — Google Drive é a fonte oficial da conta. Quando o Borion está no
   modo Google Drive, o snapshot precisa nascer do estado completo já carregado
   do current.json em memória. A função antiga consultava o Supabase sempre que
   ainda existia uma sessão antiga por lá; essa base podia estar desatualizada e
   produzir uma fotografia muito menor (ex.: 923 → 135 transações), acionando
   corretamente o Data Guard e exibindo “Salvamento bloqueado”. A proteção não
   foi removida: apenas eliminamos a fonte errada que gerava o falso positivo. */
function borionDriveIsOfficialSnapshotSource(){
  try{
    return !!(window.BorionStrictDrive&&BorionStrictDrive.isGoogleMode&&BorionStrictDrive.isGoogleMode());
  }catch(_e){return false;}
}
async function buildSharedBackupSnapshot(backupType='manual',reason=''){
  const payload=borionDriveIsOfficialSnapshotSource()
    ? await buildLocalAccountBackupPayload(backupType,reason)
    : await buildCloudAccountBackupPayload(backupType,reason);
...
const BackupFS = {
  ...
  markDirty(){
    if(window.BorionStrictDrive&&BorionStrictDrive.shouldUseMemory&&BorionStrictDrive.shouldUseMemory()) return;
    this.dirty = true;
    this.dirtyRevision++;
    this.scheduleAutoBackup();
  },
  isSupported(){ return FS_ACCESS_SUPPORTED; },
  ...
  async choose(){
    if(!FS_ACCESS_SUPPORTED){
      alert('A seleção de pasta local exige Chrome ou Edge atual, em conexão segura (HTTPS ou aplicativo instalado).');
      return false;
    }
    try{
      const handle=await window.showDirectoryPicker({id:'borion-backups-locais',mode:'readwrite',startIn:'documents'});
      let permission=await handle.queryPermission({mode:'readwrite'});
      if(permission!=='granted'&&handle.requestPermission) permission=await handle.requestPermission({mode:'readwrite'});
      if(permission!=='granted'){ toast('Permissão da pasta não concedida.'); return false; }
      let persisted=true;
      try{await idbSet('backupDir',handle);}catch(e){persisted=false;console.warn('[BORION_BACKUP][PERSIST_FOLDER_HANDLE]',e);}
      this.dirHandle=handle;
      this.pendingHandle=null;
      this.needsReconnect=false;
      this.startupFolderStatus='connected';
      this.startupFolderName=handle.name||'Pasta selecionada';
      this.initPromise=Promise.resolve(true);
      this.setConsent('folder');
      toast('Pasta local conectada: '+this.startupFolderName+(persisted?'.':' (válida nesta sessão).'));
      this.safeRefreshUI();
      return true;
    }catch(e){
      if(e&&e.name==='AbortError') return false;
      console.error('[BORION_BACKUP][CHOOSE_FOLDER]',e);
      alert('Não foi possível selecionar a pasta: '+((e&&e.message)||String(e)));
      return false;
    }
  },
  ...
  async listFolderBackups(){
    const connected=await this.verifyFolderConnection(false);
    if(!connected||!this.dirHandle) return [];
    const rows=[];
    try{
      for await (const [name,handle] of this.dirHandle.entries()){
        if(!handle||handle.kind!=='file'||!/\.json$/i.test(name)||!/^(?:borion[-_]|backup[-_])/i.test(name)) continue;
        try{const file=await handle.getFile();rows.push({name:file.name,size:file.size,lastModified:file.lastModified});}
        catch(e){console.warn('[BORION_BACKUP][LIST_FOLDER_FILE]',name,e);}
      }
    }catch(e){
      if(e&&(e.name==='NotAllowedError'||e.name==='SecurityError')){this.pendingHandle=this.dirHandle;this.dirHandle=null;this.needsReconnect=true;this.startupFolderStatus='reconnect';this.safeRefreshUI();}
      throw e;
    }
    return rows.sort((a,b)=>Number(b.lastModified||0)-Number(a.lastModified||0)||String(b.name).localeCompare(String(a.name),'pt-BR'));
  },

  async getFolderBackupFile(name){
    if(!name||/[\/]/.test(name)) throw new Error('Nome de backup inválido.');
    const connected=await this.verifyFolderConnection(true);
    if(!connected||!this.dirHandle) throw new Error('Reconecte ou escolha a pasta local de backups.');
    const handle=await this.dirHandle.getFileHandle(name,{create:false});
    return await handle.getFile();
  },

  async importFolderBackup(name){
    const file=await this.getFolderBackupFile(name);
    if(window.storageProvider&&typeof storageProvider.importJson==='function') return await storageProvider.importJson(file);
    const text=await file.text();
    let payload;try{payload=JSON.parse(text);}catch(_){throw new Error('O arquivo selecionado não é um JSON válido.');}
    const check=validateBorionJson(payload);if(!check.valid)throw new Error(check.errors.join(' '));
    handleImport(payload);return payload;
  },
