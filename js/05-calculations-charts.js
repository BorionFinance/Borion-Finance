...
function linkParcelaToDespesa(cartao, parcela){
  ...
  if(parcela){
    (S.data.transacoes||[]).filter(t=>t&&t.viaParcelaId===parcela.id&&t.tipo==='variavel').forEach(t=>{
      previousStatusByMonth[String(t.data||'').slice(0,7)]={
        status:linkedVariableStatus(t),
        manualAt:Number(t.statusPagamentoManualAt)||0,
        origin:t.statusPagamentoOrigem||''
      };
    });
  }
  ...
  if(parcela.despesaTipo==='fixa'){
    ...
    const f = {id:uid(), nome, localCompra:parcela.local||'', categoria, valor:ownMonthly, dia:parcela.diaEntrada||1, startMonth, endMonth:totalParcelas>1?endMonth:startMonth, banco:cartao.banco||'', formaPagamento:'Crédito', viaCartaoId:cartao.id, viaParcelaId:parcela.id, viaAssinaturaId:parcela.viaAssinaturaId||null, assinaturaCobrancaId:parcela.assinaturaCobrancaId||null, parcelaTotal:totalParcelas, compartilhamentoId:sharedModel?sharedModel.id:null, valorFaturaParcela:valorParcela, valorProprioTotal:sharedModel?sharedModel.valorProprioTotal:null, compartilhamentoValoresMensais:ownValues};
    S.data.fixas.push(f);
    ...
    parcela.despesaTransacaoIds.forEach(id=>{
      ...
      const previous=previousStatusByMonth[competencia];
      if(tx&&previous){
        tx.statusPagamento=previous.status;
        if(previous.manualAt)tx.statusPagamentoManualAt=previous.manualAt;
        if(previous.origin)tx.statusPagamentoOrigem=previous.origin;
      }
    });
...
function patrimonioTotal(){
  const moedas=(typeof moedasEstrangeirasTotal==='function')?moedasEstrangeirasTotal():0;
  return liquidezTotal() + reservasTotal() + bensTotal() + investAtualTotal() + moedas - computeCardsDebt().total;
}
...
function renderDonut(segments, centerTop, centerBottom, opts){
  opts = opts || {};
  const onCategoryClick = opts.onCategoryClick || null; // nome da função global, ex: 'Budget.filterByCategoria'
  const activeCategory = opts.activeCategory || null;
  const filtered = (Array.isArray(segments)?segments:[])
    .filter(s=>Number(s&&s.value)>0)
    .slice()
    .sort((a,b)=>(Number(b.value)||0)-(Number(a.value)||0)||String(a.label||'').localeCompare(String(b.label||''),'pt-BR'));
  ...
  // V7.1.3 — quando onCategoryClick é informado (hoje só usado pelas abas Receita/Despesa fixa/
  // Despesa variável), cada item da legenda vira um filtro rápido por categoria: clicar aplica o
  // filtro (o nome vai via encodeURIComponent pra não quebrar nomes com aspas) e destaca a
  // categoria ativa. Outros usos do donut (Visão Geral, Patrimônio) continuam só informativos.
  const legend = filtered.map((s,idx)=>{
    const pctTxt = total? Math.round(s.value/total*100)+'%' : '0%';
    const isActive = !!(onCategoryClick && activeCategory && activeCategory===s.label);
    const clickable = !!onCategoryClick;
    const cls = 'legend-item'+(clickable?' legend-item-clickable':'')+(isActive?' legend-item-active':'');
    const clickAttrs = clickable ? ` onclick="${onCategoryClick}('${encodeURIComponent(s.label)}')" role="button" tabindex="0" title="Filtrar por ${esc(s.label)}"` : '';
    return `
    <div class="${cls}" style="animation-delay:${(idx*0.06).toFixed(2)}s" data-label="${esc(s.label)}" data-pct="${pctTxt}" data-borion-money-value="${encodeURIComponent(brlPlain(s.value))}" data-value="${esc(brlText(s.value))}" onmousemove="ChartTooltip.showEl(event,this)" onmouseleave="ChartTooltip.hide()"${clickAttrs}>
...
/* V7.9.1 — limpeza única para tooltips dos gráficos clássicos e do Estúdio.
   É chamada em toda troca de tela/perfil e também em interações globais para que
   nenhum cartão flutuante sobreviva fora do gráfico que o criou. */
window.BorionChartTooltips = {
  hideAll(){
    ChartTooltip.hide();
    const studio=document.getElementById('cs-tooltip');
    if(studio){studio.hidden=true;studio.style.left='';studio.style.top='';}
  }
};
if(!window.__borionChartTooltipGuards){
  window.__borionChartTooltipGuards=true;
  const hideAll=()=>window.BorionChartTooltips&&window.BorionChartTooltips.hideAll();
