...
function patrimonioLiquido(){ return liquidezTotal() + reservasTotal() + investAtualTotal() + ((typeof moedasEstrangeirasTotal==='function')?moedasEstrangeirasTotal():0); }
...
function healthAnalysis(kind='monthly'){
  ...
  if(result>=0) positives.push(`Resultado positivo de ${brlPlain(result)} no período.`); else alerts.push(`Déficit de ${brlPlain(Math.abs(result))}: as despesas superaram a renda.`);
...
function dashboardWidgetHTML(key, ctx){
  ...
  if(key==='evolucaoMoedas') return (typeof renderForeignOverviewWidget==='function') ? renderForeignOverviewWidget(ctx.fxTrend) : '';
...
function overviewDashboardCatalog(){
  ...
  const labels={
    ...
    evolucaoMoedas:'Evolução das moedas estrangeiras'
  };
  const wide=new Set(['fluxoMensal','gastosCartao','resumoBanco']);
  // Borion 7.6.1 — os 18 widgets ficam sempre disponíveis no editor da própria
  // Visão Geral. A antiga aba duplicada em Configurações não controla mais o catálogo.
  const available=DEFAULT_DASHBOARD_WIDGETS.slice();
  return fixed.concat(available.map(id=>({id,label:labels[id]||id,defaultW:wide.has(id)?4:2,group:'Gráficos'})));
}
function migrateLegacyOverviewWidgetVisibility(scope){
  if(!window.ModuleLayout||!S.data) return;
  if(!S.data.uiPreferences) S.data.uiPreferences={};
  const mode=ModuleLayout.deviceMode(scope);
  const marker=(S.data.uiPreferences.overviewWidgetVisibilityMigratedV761&&typeof S.data.uiPreferences.overviewWidgetVisibilityMigratedV761==='object')
    ?S.data.uiPreferences.overviewWidgetVisibilityMigratedV761:{};
  if(marker[mode]) return;
  const legacy=(S.data.dashboard&&Array.isArray(S.data.dashboard.widgets))
    ?S.data.dashboard.widgets.filter(id=>DEFAULT_DASHBOARD_WIDGETS.includes(id))
    :DEFAULT_DASHBOARD_WIDGETS.slice();
  const layout=ModuleLayout.get(scope);
  const hidden=new Set((layout.hidden||[]).map(String));
  DEFAULT_DASHBOARD_WIDGETS.forEach(id=>{if(!legacy.includes(id))hidden.add(id);});
  layout.hidden=Array.from(hidden);
  marker[mode]=true;
  S.data.uiPreferences.overviewWidgetVisibilityMigratedV761=marker;
  ModuleLayout.save(scope,layout,{quiet:true,render:false});
}
function overviewMetricHTML(id,ctx){
  ...
  if(id==='metric_despesas'){
    const refundMini=window.Refunds&&typeof Refunds.expenseRefundMiniHTML==='function'?Refunds.expenseRefundMiniHTML(S.month.y,S.month.m,ctx.desp):'';
    return `<div class="card card-sm overview-metric-card${refundMini?' budget-expense-refund-card has-refunds':''}"${refundMini?' tabindex="0" aria-label="Despesas com detalhamento de estornos. Passe o mouse ou pressione Tab para consultar."':''}><div class="clabel">Despesas do período</div><div class="cval" style="color:${iconColor('despesas')}">${brl(ctx.desp)}</div>${refundMini}</div>`;
  }
...
function overviewSummaryHTML(ctx){
  return `<div class="panel-box overview-summary-panel">
    ...
    ${ctx.moedas>0?`<div class="list-row"><span class="lname">Moedas estrangeiras (convertidas em reais)</span><span class="lval" style="color:${iconColor('moedas')}">${brl(ctx.moedas)}</span></div>`:''}
...
function renderOverview(){
  ...
  const ctx={
    ...
    moedas:(typeof moedasEstrangeirasTotal==='function'?moedasEstrangeirasTotal():0),
    ...
    chartCtx:{
      ...
      fxTrend:(typeof foreignRatesOverviewData==='function'?foreignRatesOverviewData():null)
    }
  };
  if(ctx.moedas>0&&window.ForeignCurrencies&&typeof ForeignCurrencies.ensureOverviewData==='function')setTimeout(()=>ForeignCurrencies.ensureOverviewData(false),0);
  ...
  if(window.ModuleLayout){
    ...
    migrateLegacyOverviewWidgetVisibility(scope);
