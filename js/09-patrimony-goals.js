...
function patrimonioComposicaoSegments(){
  ...
  if(typeof foreignCurrencySegments==='function') segs.push(...foreignCurrencySegments());
...
function renderPatrimony(){
  const liq=liquidezTotal(),bens=bensTotal(),invest=investAtualTotal(),reservas=reservasTotal(),moedas=(typeof moedasEstrangeirasTotal==='function'?moedasEstrangeirasTotal():0),divDebt=computeCardsDebt(),div=divDebt.total;
  const total=liq+reservas+bens+invest+moedas-div,composicaoTotal=liq+reservas+bens+invest+moedas,segs=patrimonioComposicaoSegments();
  ...
  // V7.1.3 — Patrimônio agora usa exatamente o mesmo editor de módulos livre (arrastar,
  // redimensionar, ocultar/mostrar e sobrepor como janelas) já usado na Visão Geral, em vez do
  // modo antigo por colunas fixas. Reaproveita as mesmas classes CSS (.overview-dashboard-grid/
  // .overview-dashboard-slot) além das classes próprias do Patrimônio, pra herdar todo o estilo
  // e o comportamento de posicionamento livre sem duplicar CSS.
  const wideModules=new Set(['reservas','dividas','moedas_estrangeiras']);
  const rawModules=[
    ...
    {id:'moedas_estrangeiras',label:'Moedas estrangeiras',html:(typeof renderForeignCurrenciesPanel==='function'?renderForeignCurrenciesPanel():'')},
  ...
  ].map(m=>Object.assign({},m,{defaultW:wideModules.has(m.id)?4:2}));
  const scope='patrimony_modules';
  let visible=rawModules, columns=4, organizing=false, freePlacement=false, freeReady=true;
  if(window.ModuleLayout){
    ModuleLayout.register(scope,rawModules);
    visible=ModuleLayout.visibleItems(scope,rawModules);
    columns=ModuleLayout.get(scope).columns;
    organizing=ModuleLayout.isActive(scope);
    freePlacement=ModuleLayout.isFreePlacement(scope);
    freeReady=!freePlacement||ModuleLayout.hasCompleteFreeLayout(scope,visible);
  }
  const moduleHTML=visible.map(m=>{
    const style=window.ModuleLayout?ModuleLayout.slotStyle(scope,m.id,m.defaultW):'--module-span:'+m.defaultW+';';
    const controls=window.ModuleLayout?ModuleLayout.slotControlsHTML(scope,m.id,m.label,m.defaultW):'';
    const resize=window.ModuleLayout?ModuleLayout.resizeHandleHTML(scope,m.id,m.label):'';
    return `<div class="module-layout-slot overview-dashboard-slot patrimony-module-slot ${organizing?'organizing':''}" data-module-id="${esc(m.id)}" style="${style}">${controls}<div class="module-layout-content">${m.html}</div>${resize}</div>`;
  }).join('');
  if(window.ModuleLayout) ModuleLayout.schedule(scope);
  const emptyModules=`<div class="panel-box overview-layout-empty"><div class="empty-note">Nenhum módulo está visível neste layout. Use "Widgets" para reativar os blocos.</div></div>`;
  return `
    <div class="cards-row">
      ...
      <div class="card"><div class="clabel">${tagBadgeHTML('moedas','MOEDAS ESTRANGEIRAS')}</div><div class="cval">${brl(moedas)}</div></div>
    ...
    ${window.ModuleLayout?ModuleLayout.toolbarHTML(scope,'Organização do patrimônio'):''}
    <div class="module-layout-grid overview-dashboard-grid patrimony-modules-grid ${organizing?'module-grid-organizer':''} ${freePlacement?'module-free-placement':''} ${freePlacement&&!freeReady?'module-free-bootstrap':''}" data-module-layout="${scope}" data-free-placement="${freePlacement?'true':'false'}" style="--module-columns:${columns};">${moduleHTML||emptyModules}</div>
  `;
}
...
function reservaMoveDelta(m){
  ...
  if(['Resgatar','Pagamento direto','Pagamento de despesa fixa','Pagamento de fatura','Transferência enviada','Envio para outra reserva'].includes(m&&m.tipo)) return -v;
...
const ReservaView = {
  get(){
    if(!S.data.uiPreferences || typeof S.data.uiPreferences!=='object') S.data.uiPreferences={};
    const mode=S.data.uiPreferences.reserveViewMode;
    return mode==='list'?'list':'columns';
  },
  set(mode,event){
    if(event){event.preventDefault();event.stopPropagation();}
    const next=mode==='list'?'list':'columns';
    if(!S.data.uiPreferences || typeof S.data.uiPreferences!=='object') S.data.uiPreferences={};
    S.data.uiPreferences.reserveViewMode=next;
    saveCurrentData({skipPatrimonioSnapshot:true});
    renderView();
  },
  ...
  openHover(event){
    if(!this.supportsHover(event))return;
    this.cancelHoverClose();
    const wrap=event&&event.currentTarget?event.currentTarget:null;
    if(wrap)wrap.classList.add('is-open','is-hover-open');
  },
  scheduleHoverClose(event){
    ...
    this.hoverCloseTimer=setTimeout(()=>{
      ...
      if(wrap){wrap.classList.remove('is-open','is-hover-open');}
    },150);
  },
  toggleMenu(event){
    if(event){event.preventDefault();event.stopPropagation();}
    this.cancelHoverClose();
    const wrap=event&&event.currentTarget?event.currentTarget.closest('.reserve-view-switcher'):null;
    if(wrap){wrap.classList.toggle('is-open');wrap.classList.remove('is-hover-open');}
  },
  closeMenu(event){
    this.cancelHoverClose();
    const wrap=event&&event.currentTarget?event.currentTarget:null;
  ...
  controlHTML(){
    const mode=this.get();
    const listIcon='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 6h12M8 12h12M8 18h12"/><circle cx="4" cy="6" r="1"/><circle cx="4" cy="12" r="1"/><circle cx="4" cy="18" r="1"/></svg>';
    const columnsIcon='<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="4" width="8" height="16" rx="2"/><rect x="13" y="4" width="8" height="16" rx="2"/></svg>';
    return `<div class="reserve-view-switcher" onpointerenter="ReservaView.openHover(event)" onpointerleave="ReservaView.scheduleHoverClose(event)">
      <button type="button" class="reserve-view-orb" onclick="ReservaView.toggleMenu(event)" title="Visualização dos cofrinhos" aria-label="Escolher visualização dos cofrinhos">${mode==='list'?listIcon:columnsIcon}</button>
      <div class="reserve-view-menu" role="menu" aria-label="Visualização dos cofrinhos">
        <button type="button" class="${mode==='list'?'active':''}" onclick="ReservaView.set('list',event)" role="menuitem">${listIcon}<span>Lista</span></button>
        <button type="button" class="${mode==='columns'?'active':''}" onclick="ReservaView.set('columns',event)" role="menuitem">${columnsIcon}<span>Colunas</span></button>
      </div>
    </div>`;
  }
};
window.ReservaView=ReservaView;
...
function renderReservasPanel(){
  ...
  const reservaViewMode = window.ReservaView ? ReservaView.get() : 'columns';
  ...
  return `<div class="panel-box reservas-panel">
    <div class="toolbar reserve-main-toolbar"><div class="toolbar-left reserve-toolbar-summary">◈ <span class="lmeta">Reservado: ${brl(total)}</span><span class="reserve-active-inline">${ativas} reserva${ativas===1?'':'s'} ativa${ativas===1?'':'s'}</span></div><div class="reserva-toolbar-actions">${renderReservaReportsControls()}${window.OrderPreferences?OrderPreferences.sortSelectHTML('reservas'):''}${window.ReservaView?ReservaView.controlHTML():''}<button class="btn-outline" onclick="Reservas.add()">+ Nova reserva</button><button class="btn-outline" onclick="Reservas.move()">+ Movimentação</button></div></div>
    ${orgFilterNotice}
    ${reservaGridOrganizer ? OrderPreferences.reservaLayoutControlsHTML() : ''}
    ${boxes.length?`<div class="reserva-grid reserva-layout-custom reserva-view-${reservaViewMode} ${reservaGridOrganizer?'reserva-grid-organizer':''}" style="--reserva-cols:${reservaColumns};--reserva-card-min:${reservaColumns===4?'205px':reservaColumns===3?'230px':'280px'};" data-order-list="reservas">${boxCards}</div>`:'<div class="empty-note">Nenhuma reserva cadastrada ainda. Use para separar reserva de emergência, viagem, manutenção, impostos e objetivos.</div>'}
...
const Reservas = {
  ...
  /* V6.1 — 'Pagamento de despesa fixa' segue exatamente o mesmo mecanismo já usado por
     ...
     V7.1.4 — 'Pagamento de fatura' segue o mesmo mecanismo, usado quando uma fatura de
     cartão é paga com dinheiro de uma reserva (Cards.payFatura). */
  NEGATIVE_TYPES: ['Resgatar','Pagamento direto','Pagamento de despesa fixa','Pagamento de fatura','Transferência enviada','Envio para outra reserva'],
