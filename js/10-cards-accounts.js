...
function renderCards(){
  ...
  const accRows = contasOrdered.map(a=>`
    <div class="card-entity" data-order-id="${esc(a.id)}">
      <div class="card-entity-head ${showReorderContas?'account-entity-head-reorder':'account-entity-head'}">
        <div class="cehl">
          <div class="bank-badge ${a.isCarteira?'wallet-bank-badge':''}" style="background:${a.isCarteira?'#f3f7ef':(a.cor||bankColor(a.nome))}">${a.isCarteira?'<img src="assets/icons/borion-cash-wallet.png" alt="Dinheiro em espécie" class="wallet-cash-icon">':esc(a.icone||(a.nome||'?')[0])}</div>
        ...
        ${showReorderContas ? OrderPreferences.reorderRowControlsHTML('contas', a.id, a.nome, contasNaturalIds) : `<div style="display:flex;gap:8px;"><button type="button" class="btn-outline btn-sm" onclick="Cards.viewExtrato('${a.id}')" title="Ver extrato" aria-label="Ver extrato">Ver extrato</button><button type="button" class="btn-outline btn-sm account-edit-btn" onclick="Cards.editConta('${a.id}')" title="Editar conta" aria-label="Editar conta"><span class="account-edit-icon" aria-hidden="true">✎</span><span class="account-edit-label">Editar</span></button></div>`}
      </div>
    </div>`).join('');
  ...
  return `
    ...
    <p style="font-size:11.5px;color:var(--muted-2);margin:-6px 0 12px;">Compras no cartão viram parcelas dentro da fatura e não mexem no saldo do banco — cada compra já é uma despesa lançada por si só. Pagar a fatura desconta o valor da Carteira, Conta ou Reserva escolhida e não gera uma despesa nova.</p>
...
/* V7.1.4 — extrato de uma conta ou da Carteira: reúne tudo que já mexeu no saldo dela, no
   mesmo espírito do extrato de reserva (Reservas.viewExtrato) — receitas recebidas, despesas
   variáveis/fixas pagas direto por ela, transferências, boletos e faturas de cartão pagas com
   ela. Nunca inclui a compra parcelada em si (isso só afeta a fatura, não o saldo da conta). */
function accountExtratoEntries(accountId){
  if(!accountId) return [];
  const entries=[];
  (S.data.transacoes||[]).forEach(t=>{
    if(t.tipo==='receita' && t.accountId===accountId){
      entries.push({data:t.data||'', tipo:'Receita', descricao:t.nome||'Receita', valor:Number(t.valor)||0, createdAt:Number(t.addedAt||t.createdAt)||0});
    }else if(t.tipo==='variavel' && t.accountId===accountId && t.statusPagamento!=='Em aberto' && t.origemPagamento!=='reserva' && t.formaPagamento!=='Crédito'){
      entries.push({data:t.data||'', tipo:'Despesa variável', descricao:t.nome||'Despesa variável', valor:-(Number(t.valor)||0), createdAt:Number(t.addedAt||t.createdAt)||0});
    }
  });
  (S.data.fixaPagamentos||[]).forEach(r=>{
    if(!r.pago || r.origemPagamento!=='conta' || r.accountId!==accountId) return;
    const f=(S.data.fixas||[]).find(x=>x.id===r.fixaId);
    const dia=Math.max(1,Math.min(28,Number((f&&f.dia)||1)));
    const data=r.mesKey ? r.mesKey+'-'+pad2(dia) : '';
    entries.push({data, tipo:'Despesa fixa', descricao:(f&&f.nome)||'Despesa fixa', valor:-(Number(r.valorPago)||0), createdAt:Number(r.pagoEm)||0});
  });
  (S.data.transferencias||[]).forEach(t=>{
    const origemId=t.origemTipo==='conta'?(t.origemAccountId||t.origemId):null;
    const destinoId=t.destinoTipo==='conta'?(t.destinoAccountId||t.destinoId):null;
    if(origemId===accountId){
      const saque=destinoId===CARTEIRA_CONTA_ID&&origemId!==CARTEIRA_CONTA_ID;
      const deposito=origemId===CARTEIRA_CONTA_ID&&destinoId!==CARTEIRA_CONTA_ID;
      entries.push({data:t.data||'', tipo:saque?'Saque':(deposito?'Depósito em conta':'Transferência'), descricao:t.descricao||('Transferência para '+(t.destinoNome||'')), valor:-(Number(t.valor)||0), createdAt:Number(t.createdAt)||0});
    }
    if(destinoId===accountId){
      const daReserva=t.origemTipo==='reserva';
      const recebidoNaCarteira=destinoId===CARTEIRA_CONTA_ID;
      entries.push({data:t.data||'', tipo:recebidoNaCarteira?(daReserva?'Saque da reserva':'Dinheiro em espécie'):(daReserva?'Resgate de reserva':'Transferência'), descricao:t.descricao||('Transferência de '+(t.origemNome||'')), valor:Number(t.valor)||0, createdAt:Number(t.createdAt)||0});
    }
  });
  (S.data.boletos||[]).forEach(b=>{
    (b.pagamentos||[]).forEach(p=>{
      if(p.accountId!==accountId) return;
      entries.push({data:p.data||'', tipo:'Boleto', descricao:'Boleto — '+(b.descricao||b.credor||'Boleto'), valor:-(Number(p.valor)||0), createdAt:Number(p.createdAt)||0});
    });
  });
  (S.data.cartoes||[]).forEach(c=>{
    (c.faturasPagas||[]).forEach(f=>{
      if(f.informationalOnly || f.accountId!==accountId) return;
      entries.push({data:f.data||'', tipo:'Fatura de cartão', descricao:'Fatura — '+(c.banco||'Cartão'), valor:-(Number(f.valor)||0), createdAt:Number(f.createdAt)||0});
    });
  });
  return entries.sort((a,b)=> String(b.data||'').localeCompare(String(a.data||'')) || (Number(b.createdAt)||0)-(Number(a.createdAt)||0));
}
const Cards = {
  addConta(){
  ...
  /* V7.1.4 — extrato da conta/carteira em modal, mesmo padrão do extrato de reserva:
     período, tipo de movimentação e busca, com totais de entradas/saídas do filtro atual. */
  viewExtrato(accountId){
    const account = accountById(accountId,{includeArchived:true});
    if(!account){ toast('Conta não encontrada.'); return; }
    const state={periodo:'todos', dataDe:'', dataAte:'', tipo:'todos', busca:''};
    const allEntries = accountExtratoEntries(accountId);
    const tiposPresentes = Array.from(new Set(allEntries.map(e=>e.tipo))).sort();
    const box_ = el(`
      <div class="modal-overlay">
        <div class="modal-box" style="max-width:640px;">
          <div class="modal-head"><h2>Extrato — ${esc(account.nome)}</h2><button id="cax_close">&times;</button></div>
          <p class="modal-sub">Só movimentações desta ${account.isCarteira?'carteira':'conta'}. Nunca mistura com outra conta ou reserva.</p>
          <div class="cards-row" style="margin-bottom:10px;">
            <div class="card"><div class="clabel">Saldo atual</div><div class="cval">${brl(contaSaldoAtual(account))}</div></div>
            <div class="card"><div class="clabel">Entradas no período</div><div class="cval val-pos" id="cax_in">—</div></div>
            <div class="card"><div class="clabel">Saídas no período</div><div class="cval" id="cax_out">—</div></div>
          </div>
          <div class="field"><label>Buscar</label><input type="text" id="cax_busca" placeholder="Buscar por tipo ou descrição..."/></div>
          <div class="field"><label>Tipo de movimentação</label><select id="cax_tipo"><option value="todos">Todos</option>${tiposPresentes.map(t=>`<option value="${esc(t)}">${esc(t)}</option>`).join('')}</select></div>
          <div class="field"><label>Período</label><select id="cax_periodo">${PERIODO_QUICK_OPTIONS.map(o=>`<option value="${o.v}">${o.l}</option>`).join('')}</select></div>
          <div id="cax_custom_wrap" class="hidden" style="display:flex;gap:8px;">
            <div class="field" style="flex:1;"><label>De</label><input type="date" id="cax_de"/></div>
            <div class="field" style="flex:1;"><label>Até</label><input type="date" id="cax_ate"/></div>
          </div>
          <div id="cax_list_wrap"></div>
        </div>
      </div>`);
    $('#modal-root').innerHTML=''; $('#modal-root').appendChild(box_);
    attachModalGuard(box_);
    $('#cax_close').onclick = closeModal;
    ...
    function renderList(){
      ...
      if(range.de) list = list.filter(e=>(e.data||'')>=range.de);
      if(range.ate) list = list.filter(e=>(e.data||'')<=range.ate);
      if(state.tipo!=='todos') list = list.filter(e=>e.tipo===state.tipo);
      if(q) list = list.filter(e=>[e.tipo,e.descricao].filter(Boolean).join(' ').toLowerCase().includes(q));
      let entradas=0, saidas=0;
      list.forEach(e=>{ if(e.valor>0)entradas+=e.valor; else if(e.valor<0)saidas+=Math.abs(e.valor); });
      $('#cax_in').textContent = brlText(entradas);
      $('#cax_out').textContent = S.valuesHidden?'$':'- '+brlPlain(saidas).replace('R$ -','R$ ');
      const rows = list.map(e=>`<tr>
        <td>${reservaFmtDate(e.data)}</td>
        <td>${esc(e.tipo)}</td>
        <td>${reservaSignedValue(e.valor)}</td>
        <td>${esc(e.descricao||'')}</td>
      </tr>`).join('');
      $('#cax_list_wrap').innerHTML = rows ? `<div class="table-scroll"><table><thead><tr><th>Data</th><th>Tipo</th><th>Valor</th><th>Descrição</th></tr></thead><tbody>${rows}</tbody></table></div>` : '<div class="empty-note">Nenhuma movimentação encontrada para esse filtro.</div>';
    }
    $('#cax_busca').oninput = ()=>{ state.busca = $('#cax_busca').value; renderList(); };
    $('#cax_tipo').onchange = ()=>{ state.tipo = $('#cax_tipo').value; renderList(); };
    $('#cax_periodo').onchange = ()=>{
      state.periodo = $('#cax_periodo').value;
      $('#cax_custom_wrap').classList.toggle('hidden', state.periodo!=='personalizado');
      renderList();
    };
    $('#cax_de').onchange = ()=>{ state.dataDe = $('#cax_de').value; renderList(); };
    $('#cax_ate').onchange = ()=>{ state.dataAte = $('#cax_ate').value; renderList(); };
    renderList();
  },
  editConta(id){
  ...
  editCartao(id){
    ...
    onDelete(){
      const removed=(c.parcelas||[]).slice();
      removed.forEach(p=>unlinkParcelaFromDespesa(p));
      S.data.cartoes = S.data.cartoes.filter(x=>x.id!==id);
      if(window.Assinaturas&&typeof Assinaturas.onCardPurchaseDeleted==='function'){
        removed.forEach(p=>Assinaturas.onCardPurchaseDeleted(p,c,{cardDeleted:true}));
      }
      saveCurrentData(); closeModal(); renderView();
    },
    onSave(v){
  ...
  addParcela(cartaoId){
    openModal({title:'Adicionar compra parcelada', sub:'A parcela atual é calculada automaticamente de acordo com o mês da compra e o mês selecionado no calendário. Compras no cartão não mudam o saldo do banco — só a fatura. Pagar a fatura desconta o valor de onde você escolher.', fields:[
      ...
      const p = {id:uid(), descricao:v.descricao, local:v.local, categoria:v.categoria||'Outro', valorParcela, parcelaTotal, dataCompra:dataCompraCompleta.slice(0,7), dataCompraCompleta, diaEntrada:diaCompra, apareceDespesas:sharedResult.model?sharedResult.model.valorProprioTotal>0:!!v.apareceDespesas, despesaTipo:v.despesaTipo||'variavel', statusPagamento:'Em aberto', statusFaturaPorCompetencia:{}, despesaTransacaoId:null, despesaTransacaoIds:[], despesaFixaId:null, compartilhamento:sharedResult.model};
      c.parcelas.push(p);
      if(typeof assinaturaRegistrarPreferenciaDespesas==='function')assinaturaRegistrarPreferenciaDespesas(p,p.apareceDespesas);
  ...
  editParcela(cartaoId, parcelaId){
    ...
    onDelete(){
      ...
      unlinkParcelaFromDespesa(p);
      c.parcelas = c.parcelas.filter(x=>x.id!==parcelaId);
      if(window.Assinaturas&&typeof Assinaturas.onCardPurchaseDeleted==='function') Assinaturas.onCardPurchaseDeleted(p,c);
      saveCurrentData(); closeModal(); renderView();
    },
    onSave(v){
      ...
      /* V7.9.5 — se esta compra veio de uma assinatura, a caixa marcada aqui precisa ficar
         gravada na assinatura, senao a reconciliacao que roda a cada troca de aba desfaz. */
      if(typeof assinaturaRegistrarPreferenciaDespesas==='function')assinaturaRegistrarPreferenciaDespesas(p,p.apareceDespesas);
      linkParcelaToDespesa(c, p);
      saveCurrentData(); closeModal(); renderView();
    }});
    /* A visibilidade é uma preferência, não parte do formulário inteiro. Ela entra
       no estado local no próprio clique; o Drive confirma depois, em segundo plano.
       Assim fechar o modal ou trocar de aba não cancela nem reverte a escolha. */
    const instantExpenseVisibility=document.getElementById('mf_apareceDespesas');
    if(instantExpenseVisibility)instantExpenseVisibility.addEventListener('change',()=>{
      const visible=instantExpenseVisibility.checked;
      const ok=runAtomicFinancialMutation(()=>{
        p.apareceDespesas=visible;
        if(typeof assinaturaRegistrarPreferenciaDespesas==='function')assinaturaRegistrarPreferenciaDespesas(p,visible);
        if(visible)linkParcelaToDespesa(c,p);else unlinkParcelaFromDespesa(p);
      },()=>alert('Não foi possível atualizar a visibilidade. O estado anterior foi preservado.'));
      if(!ok){closeModal();renderView();return;}
      saveCurrentData({source:'card_expense_visibility'});
      renderView();
      toast(visible?'A compra já aparece em Despesas; salvando no Drive.':'A compra já saiu de Despesas; salvando no Drive.');
    });
    if(window.SharedPurchases)SharedPurchases.bindForm({prefix:'mf',existing:p,totalResolver:()=>{const input=document.getElementById('mf_valorParcela'),qtd=document.getElementById('mf_parcelaTotal');return (parseInt(input&&input.dataset.cents||'0',10)/100)*Math.max(1,Math.round(Number(qtd&&qtd.value)||1));},installmentsResolver:()=>Math.max(1,Math.round(Number(document.getElementById('mf_parcelaTotal')&&document.getElementById('mf_parcelaTotal').value)||1))});
    wireParcelaCategoriaPorTipo(p.categoria);
  },
  /* V7.1.4 — Pagar fatura passou a ser uma movimentação financeira real (antes era só um
     registro informativo). Escolhe de onde sai o dinheiro — Carteira, Conta ou Reserva — e:
     - Carteira/Conta: desconta via adjustLiquidez (mesmo mecanismo de qualquer despesa paga
       direto da conta).
     - Reserva: cria uma movimentação "Pagamento de fatura" no extrato da reserva (mesmo
       mecanismo de Pagamento de despesa fixa/Pagamento direto).
     Nunca cria nada em despesas fixas/variáveis — cada compra da fatura já está lançada
     individualmente como parcela do cartão. */
  payFatura(cartaoId){
    const c = S.data.cartoes.find(x=>x.id===cartaoId);
    if(!c) return;
    ...
    const initialSource = bankAccounts.length?'conta':(boxes.length?'reserva':'carteira');
    ...
    const box = el(`<div class="modal-overlay"><div class="modal-box">
      <div class="modal-head"><h2>Pagar fatura</h2><button id="pf_close">&times;</button></div>
      <p class="modal-sub">Fatura de ${esc(monthLabel(S.month.y,S.month.m))} do cartão ${esc(c.banco)}. O valor sai de onde você escolher abaixo. As compras já lançadas individualmente na fatura não mudam.</p>
      <div class="field"><label>Valor da fatura (R$)</label><input type="text" inputmode="numeric" class="money-input" id="pf_valor" placeholder="0,00"/></div>
      <div class="field"><label>De onde será pago</label><div class="segmented-toggle payment-source-toggle" id="pf_source_group">
      ...
      </div><input type="hidden" id="pf_source" value="${initialSource}"/></div>
      <div id="pf_wallet_panel" class="payment-source-panel ${initialSource==='carteira'?'':'hidden'}">
        <div class="info-box">Pago com dinheiro da Carteira.</div>
      </div>
      <div id="pf_account_panel" class="payment-source-panel ${initialSource==='conta'?'':'hidden'}">
        <div class="field"><label>Conta (Pix ou débito)</label><select id="pf_account">${accountOptions||'<option value="">Cadastre uma conta bancária</option>'}</select></div>
      </div>
      <div id="pf_reserve_panel" class="payment-source-panel ${initialSource==='reserva'?'':'hidden'}">
        <div class="field"><label>Reserva</label><select id="pf_reserve">${boxOptions||'<option value="">Crie uma reserva primeiro</option>'}</select></div>
      </div>
      <div class="field"><label>Data do pagamento</label><input type="date" id="pf_data" value="${todayISO()}"/></div>
      <div class="row-btns"><button class="btn btn-primary btn-block" id="pf_save">Pagar</button></div>
    </div></div>`);
    $('#modal-root').innerHTML=''; $('#modal-root').appendChild(box); attachModalGuard(box);
    $('#pf_close').onclick=closeModal;
    attachMoneyMask($('#pf_valor'), info.total);
    ...
    function syncSourceUI(source){
      // V7.1.5 — os ids reais dos painéis (pf_wallet_panel/pf_account_panel/pf_reserve_panel)
      // são em inglês; o mapa abaixo evita o bug de tentar achar "pf_conta_panel"/
      // "pf_reserva_panel" (que não existem) e a troca de painel não acontecer.
      const panelIdByValue={carteira:'wallet', conta:'account', reserva:'reserve'};
      Object.keys(panelIdByValue).forEach(k=>{const node=$('#pf_'+panelIdByValue[k]+'_panel');if(node)node.classList.toggle('hidden',source!==k);});
    }
    wireSegmented('#pf_source_group','#pf_source',syncSourceUI);
    syncSourceUI(initialSource);
    $('#pf_save').onclick=()=>{
      const source=$('#pf_source').value;
      const data=$('#pf_data').value||todayISO();
      const valor=parseInt($('#pf_valor').dataset.cents||'0',10)/100;
      if(valor<=0){ alert('Digite um valor maior que zero.'); return; }
      let accountId=null, reservaBox=null;
      if(source==='carteira'){
        accountId=requireAccountId(CARTEIRA_CONTA_ID,'A Carteira precisa estar disponível.'); if(!accountId) return;
      }else if(source==='conta'){
        accountId=requireAccountId($('#pf_account').value,'Escolha a conta que vai pagar a fatura.'); if(!accountId) return;
      }else{
        reservaBox=boxes.find(r=>r.id===($('#pf_reserve')&&$('#pf_reserve').value));
        if(!reservaBox){ alert('Escolha a reserva que vai pagar a fatura.'); return; }
        if(!reservaTemSaldo(reservaBox,valor)){ showReservaInsuficienteModal(reservaBox,valor); return; }
      }
      const ok=runAtomicFinancialMutation(()=>{
        if(!c.faturasPagas) c.faturasPagas=[];
        let pagamento;
        if(reservaBox){
          const mv={id:uid(), boxId:reservaBox.id, tipo:'Pagamento de fatura', data, valor, banco:reservaBox.banco||'', descricao:'Pagamento da fatura — '+(c.banco||'Cartão'), createdAt:Date.now()};
          Reservas.applyMoveEffect(mv);
          S.data.reservas.moves.push(mv);
          pagamento={id:uid(), competencia:info.competencia, valor, origemTipo:'reserva', reservaId:reservaBox.id, reservaMoveId:mv.id, accountId:null, banco:reservaBox.nome+' (reserva)', data, createdAt:Date.now()};
        }else{
          if(!adjustLiquidez(accountId,-valor)) throw new Error('conta_indisponivel');
          pagamento={id:uid(), competencia:info.competencia, valor, origemTipo:(accountId===CARTEIRA_CONTA_ID?'carteira':'conta'), reservaId:null, reservaMoveId:null, accountId, banco:accountNameSnapshot(accountId), data, createdAt:Date.now()};
        }
        c.faturasPagas.push(pagamento);
      },()=>{ alert('Não foi possível registrar o pagamento da fatura. Nada foi alterado.'); });
      if(!ok) return;
      saveCurrentData(); closeModal(); renderView(); toast('Fatura paga.');
    };
  },
  undoFaturaPagamento(cartaoId, pagamentoId){
    ...
    // Registros antigos (de antes da v7.1.4) eram só informativos e nunca mexeram em saldo.
    const legacyInformational = pg.informationalOnly===true;
    const text = legacyInformational
      ? 'Voltar esta fatura para em aberto? Esta ação altera somente a informação exibida e não movimenta saldos.'
      : 'Voltar esta fatura para em aberto? O valor pago volta para a carteira, conta ou reserva usada no pagamento.';
    openConfirmModal({title:'Desfazer pagamento da fatura', text, confirmLabel:'Voltar para em aberto', variant:'danger', onConfirm(){
      const ok=runAtomicFinancialMutation(()=>{
        if(!legacyInformational){
          if(pg.origemTipo==='reserva' && pg.reservaMoveId){
            const midx=(S.data.reservas.moves||[]).findIndex(m=>m.id===pg.reservaMoveId);
            if(midx>=0){ Reservas.reverseMoveEffect(S.data.reservas.moves[midx]); S.data.reservas.moves.splice(midx,1); }
          }else if(pg.accountId){
            adjustLiquidez(pg.accountId, pg.valor);
          }
        }
        c.faturasPagas.splice(idx,1);
      },()=>{ alert('Não foi possível desfazer o pagamento. Nada foi alterado.'); });
      if(!ok) return;
      saveCurrentData(); renderView(); toast(legacyInformational?'Fatura voltou para em aberto. Nenhum saldo foi alterado.':'Fatura voltou para em aberto. O valor voltou para a origem do pagamento.');
    }});
  },
  ...
  /* ---------------- V7.7.3 — Transferências unificadas + saques ----------------
     Esta é a única janela de movimentação usada por Lançamentos → Transferências e por
     Reserva → + Movimentação. Conta → Carteira representa saque; Reserva pode resgatar
     para qualquer Conta ou para a Carteira, sem criar Receita nem Despesa. */
  ...
  editTransferencia(id,options={}){
    ...
    let initialAccountDestType=isEdit&&t.destinoTipo==='reserva'
      ?'reserva'
      :(isEdit&&t.destinoTipo==='conta'&&(t.destinoAccountId||t.destinoId)===CARTEIRA_CONTA_ID?'carteira':'conta');
    const initialReserveDestType=isEdit&&t.origemTipo==='reserva'&&t.destinoTipo==='conta'&&(t.destinoAccountId||t.destinoId)===CARTEIRA_CONTA_ID
      ?'carteira'
      :(bankAccounts.length?'conta':'carteira');
    const initialReserveDestAccount=isEdit&&t.origemTipo==='reserva'&&t.destinoTipo==='conta'&&(t.destinoAccountId||t.destinoId)!==CARTEIRA_CONTA_ID
      ?(t.destinoAccountId||t.destinoId)
      :(initialOriginBox?boxAccountId(initialOriginBox):((bankAccounts[0]||{}).value||''));
    ...
    const box=el(`<div class="modal-overlay"><div class="modal-box transfer-modal-box">
      ...
      <p class="modal-sub">A mesma janela é usada em Lançamentos e em Reserva. Movimentações entre Carteira, Contas e Reservas não contam como receita nem despesa.</p>
      ...
      <div id="tr_account_panel" class="payment-source-panel ${initialSource==='conta'?'':'hidden'}">
        ...
        <div class="field"><label>Destino</label><div class="segmented-toggle" id="tr_account_dest_type_group">
          ...
          <button type="button" class="seg-btn ${initialAccountDestType==='carteira'?'active':''}" data-value="carteira">Carteira</button>
        ...
        <div id="tr_account_to_wallet" class="payment-source-panel ${initialAccountDestType==='carteira'?'':'hidden'}"><div class="info-box">O valor sairá da Conta e entrará na <b>Carteira</b> como dinheiro em espécie. Esta movimentação será identificada como saque.</div></div>
      ...
      <div id="tr_reserve_panel" class="payment-source-panel ${initialSource==='reserva'?'':'hidden'}">
        ...
        <div id="tr_resgate_fields" class="payment-source-panel ${initialReserveAction==='resgatar'?'':'hidden'}">
          <div class="field"><label>Destino do resgate</label><div class="segmented-toggle" id="tr_resgate_dest_type_group">
            <button type="button" class="seg-btn ${initialReserveDestType==='conta'?'active':''}" data-value="conta" ${bankAccounts.length?'':'disabled'}>Conta</button>
            <button type="button" class="seg-btn ${initialReserveDestType==='carteira'?'active':''}" data-value="carteira">Carteira</button>
          </div><input type="hidden" id="tr_resgate_dest_type" value="${initialReserveDestType}"/></div>
          <div id="tr_resgate_to_account" class="payment-source-panel ${initialReserveDestType==='conta'?'':'hidden'}"><div class="field"><label>Conta que receberá</label><select id="tr_resgate_dest_account">${accountOptions||'<option value="">Cadastre uma conta bancária</option>'}</select></div></div>
          <div id="tr_resgate_to_wallet" class="payment-source-panel ${initialReserveDestType==='carteira'?'':'hidden'}"><div class="info-box">O valor sairá da Reserva e entrará na <b>Carteira</b> como dinheiro em espécie. Isto representa um saque, não uma receita.</div></div>
          <div class="info-box" id="tr_resgate_account_info"></div>
        </div>
    ...
    if($('#tr_resgate_dest_account'))$('#tr_resgate_dest_account').value=initialReserveDestAccount;
    ...
    function syncReserveDestinationOptions(){
      ...
      const destSelect=$('#tr_resgate_dest_account');
      if(destSelect&&!destSelect.value&&accountId&&bankAccounts.some(o=>o.value===accountId))destSelect.value=accountId;
      const info=$('#tr_resgate_account_info');if(info)info.innerHTML=account
        ?`Conta originalmente vinculada à Reserva: <b>${esc(account.nome)}</b>. Você pode manter essa conta, escolher outra ou sacar para a Carteira.`
        :'Esta Reserva não possui uma conta vinculada disponível, mas ainda pode ser resgatada para outra Conta ou para a Carteira.';
    }
    function syncSourceUI(source){
      // V7.1.5 — mesma correção do payFatura: os ids reais dos painéis (tr_wallet_panel/
      // tr_account_panel/tr_reserve_panel) são em inglês; o mapa evita procurar por
      // "tr_conta_panel"/"tr_reserva_panel" (que não existem), o que fazia o painel nunca
      // trocar ao clicar em Conta ou Reserva depois da abertura do modal.
      const panelIdByValue={carteira:'wallet', conta:'account', reserva:'reserve'};
      Object.keys(panelIdByValue).forEach(k=>{const node=$('#tr_'+panelIdByValue[k]+'_panel');if(node)node.classList.toggle('hidden',source!==k);});
      syncValueLabel();
    }
    function syncAccountDestUI(dest){
      if($('#tr_account_to_account'))$('#tr_account_to_account').classList.toggle('hidden',dest!=='conta');
      if($('#tr_account_to_wallet'))$('#tr_account_to_wallet').classList.toggle('hidden',dest!=='carteira');
    ...
    function syncResgateDestUI(dest){
      if($('#tr_resgate_to_account'))$('#tr_resgate_to_account').classList.toggle('hidden',dest!=='conta');
      if($('#tr_resgate_to_wallet'))$('#tr_resgate_to_wallet').classList.toggle('hidden',dest!=='carteira');
    }
    ...
    wireSegmented('#tr_resgate_dest_type_group','#tr_resgate_dest_type',syncResgateDestUI);
    ...
    syncAccountReserveOptions();syncReserveDestinationOptions();syncSourceUI(initialSource);syncAccountDestUI(initialAccountDestType);syncReserveActionUI(initialReserveAction);syncResgateDestUI(initialReserveDestType);
    ...
    $('#tr_save').onclick=()=>{
      ...
      if(source==='carteira'){
        ...
        }else if(destType==='carteira'){
          obj={kind:'transferencia',origemTipo:'conta',origemId,origemAccountId:origemId,origemNome:accountNameSnapshot(origemId),destinoTipo:'conta',destinoId:CARTEIRA_CONTA_ID,destinoAccountId:CARTEIRA_CONTA_ID,destinoNome:accountNameSnapshot(CARTEIRA_CONTA_ID,'Carteira'),reservaAction:null,valor,data,descricao:descricao||'Saque para dinheiro em espécie'};
        }else{
        ...
        if(action==='resgatar'){
          const destType=$('#tr_resgate_dest_type').value;
          const destinoId=destType==='carteira'?CARTEIRA_CONTA_ID:$('#tr_resgate_dest_account').value;
          if(!destinoId){alert('Escolha a Conta que receberá o resgate.');return;}
          obj={kind:'transferencia',origemTipo:'reserva',origemId:origem.id,origemAccountId:null,origemNome:origem.nome,destinoTipo:'conta',destinoId,destinoAccountId:destinoId,destinoNome:accountNameSnapshot(destinoId,destType==='carteira'?'Carteira':''),reservaAction:'resgatar',valor,data,descricao:descricao||(destType==='carteira'?'Saque da reserva para dinheiro em espécie':'Resgate da reserva')};
