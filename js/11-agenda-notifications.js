...
const Agenda = {
  ...
  edit(id){
    ...
    onSave(v){
      ...
      if(a.pago && !wasPago){
        // V7.1.3 — mesma correção do togglePago: remove a notificação em vez de só marcar lida.
        S.data.notificacoes = (S.data.notificacoes||[]).filter(n=>n.lembreteId!==a.id);
      }
  ...
  togglePago(id){
    ...
    if(!a) return;
    a.pago = !a.pago;
    // V7.1.3 — marcar como paga remove a notificação do sino em vez de só marcar como lida,
    // pra não ficar acumulando. Se desmarcar (voltar a "em aberto") e o lembrete continuar
    // vencido/atrasado, o Notifs.refresh() recria a notificação normalmente.
    if(a.pago){ S.data.notificacoes = (S.data.notificacoes||[]).filter(n=>n.lembreteId!==a.id); }
...
const Notifs = {
  ...
  openHover(evt){
    ...
    if(this.panelOpen && this.panelMode==='click') return;
    if(this.panelOpen && this.panelMode==='hover') return;
    this.panelOpen=true;
  ...
  scheduleHoverClose(){
    ...
    this.hoverCloseTimer=setTimeout(()=>{
      ...
      if(this.panelOpen && this.panelMode==='hover') this.closePanel({immediate:true});
    },this.hoverDelayMs);
  },
  togglePanel(evt){
  ...
  finishPanelClose(){
    ...
    document.body.style.removeProperty('--notif-overlay-opacity');
  ...
  closePanel(options={}){
    ...
    /* Mantém exatamente a posição atual do gesto e só então inicia a saída. Isso evita o
       salto seco que ocorria ao retirar a classe sem transição e aplicar 106% no mesmo frame. */
    const currentY=panel.style.getPropertyValue('--notif-sheet-y')||'0px';
    panel.classList.remove('is-panel-dragging','is-panel-settling');
    panel.classList.add('is-panel-closing');
    panel.style.setProperty('--notif-sheet-y',currentY);
    document.body.style.setProperty('--notif-overlay-opacity',document.body.style.getPropertyValue('--notif-overlay-opacity')||'1');
    void panel.offsetHeight;

    const finishOnTransition=event=>{
      if(event.target!==panel||event.propertyName!=='transform')return;
      panel.removeEventListener('transitionend',finishOnTransition);
      this.finishPanelClose();
    };
    panel.addEventListener('transitionend',finishOnTransition);
    requestAnimationFrame(()=>requestAnimationFrame(()=>{
      if(!panel.isConnected) return;
      panel.style.setProperty('--notif-sheet-y','106%');
      document.body.style.setProperty('--notif-overlay-opacity','0');
      document.body.classList.add('notif-panel-closing');
    }));
    this._panelCloseTimer=setTimeout(()=>this.finishPanelClose(),520);
  },
  renderPanel(){
    ...
    panel.classList.remove('is-panel-closing','is-panel-settling','is-panel-dragging');
    panel.style.removeProperty('--notif-sheet-y');
    document.body.style.setProperty('--notif-overlay-opacity','1');
    ...
    const items=list.map(n=>{
      ...
      return `<div class="notif-swipe-shell" data-notif-id="${esc(n.id)}">
        <div class="notif-delete-bg" aria-hidden="true"></div>
        <article class="notif-item ${n.lida?'':'unread'}" tabindex="0">
          ...
          <span class="ni-actions">
            <button onclick="Notifs.toggleRead('${n.id}')">${n.lida?'Marcar como não lida':'Marcar como lida'}</button>
    ...
    const shouldLock=this.panelMode!=='hover' || (typeof isSmartphoneMode==='function' && isSmartphoneMode());
    document.body.classList.toggle('notif-panel-open',shouldLock);
    if(shouldLock && typeof syncGlobalScrollLockState==='function') syncGlobalScrollLockState({source:'Notifs.open'});
  ...
  closePanelOnOutsideClick(){
    ...
    document.addEventListener('click', ()=>{ if(Notifs.panelOpen) Notifs.closePanel(); });
  }
};
