...
function hasAnyPendingExitWork(){
  try{
    if(S&&S.currentProfile&&S.data&&typeof hasExitSavePending==='function'&&hasExitSavePending(S.currentProfile.id))return true;
    const provider=window.GoogleDriveProvider||null;
    if(provider&&(provider.dirty||provider._syncInFlight||provider._strictCommitPromise||provider.hasPersistedPending&&provider.hasPersistedPending()||provider.hasPersistedConsolidation&&provider.hasPersistedConsolidation()))return true;
    if(window.BorionStrictPendingRecovery&&provider&&BorionStrictPendingRecovery.has(provider.folderId))return true;
  }catch(e){}
  return false;
}
...
const ExitSaveGuard = {
  ...
  finalSaveSilently(reason='exit'){
    try{
      ...
      if(!hasAnyPendingExitWork()) return;
      // v7.0 SECURITY — dispara primeiro as cópias locais duráveis, sem depender
      // de token ou rede. O fluxo Drive estrito da v6.46.67 continua igual.
      if(window.BorionFastWal700&&S&&S.currentProfile&&S.data)BorionFastWal700.persistCurrent({reason:'exit_'+reason});
      if(window.BorionSecurity700&&S&&S.currentProfile&&S.data)BorionSecurity700.captureCurrent('exit_'+reason,{immediate:true});
      if(window.BorionStrictDrive&&BorionStrictDrive.shouldUseMemory()){
        if(window.BorionStrictPendingRecovery)BorionStrictPendingRecovery.persist('exit_'+reason);
        if(window.GoogleDriveProvider&&GoogleDriveProvider.isConnected()){
          if(typeof GoogleDriveProvider.joinOrRequestStrictCommit==='function')GoogleDriveProvider.joinOrRequestStrictCommit('exit_'+reason);
          else GoogleDriveProvider.syncNow({source:'exit_'+reason});
        }
        return;
      }
...
window.addEventListener('beforeunload', e=>{
  ...
  // v7.7.9 — o modo cloud-only (v7.0 Security) não guarda mais uma cópia local de
  // segurança: o salvamento acima depende só da gravação no Google Drive terminar.
  // Sem aviso nenhum aqui, fechar rápido demais podia cortar essa gravação no meio
  // e a mudança feita por último se perdia (ou "voltava" ao reabrir em outro
  // dispositivo). O diálogo nativo do navegador dá tempo real pro salvamento em
  // andamento terminar em segundo plano enquanto a pessoa decide sair ou não.
  e.preventDefault();
  e.returnValue = 'O Borion ainda está salvando na nuvem. Aguarde alguns segundos antes de fechar.';
...
function registerBorionServiceWorker642(){
  ...
  borionServiceWorkerPromise=(async()=>{
    ...
    try{
      const releaseId=window.BORION_RELEASE_ID||'borion-7.9.7-salvamento-drive-resiliente';
      const registration=await navigator.serviceWorker.register(`./sw.js?v=${encodeURIComponent(releaseId)}`,{scope:'./',updateViaCache:'none'});
      // A verificação ocorre em segundo plano e nunca bloqueia login, Drive ou edição.
      registration.update().catch(()=>{});
      return registration;
    }catch(e){
      console.warn('[BORION][PWA] service worker indisponível; o aplicativo segue normalmente:',e);
      return null;
    }
  })();
...
/* V7.8.2 — RECUPERAÇÃO SILENCIOSA ANTES DE MOSTRAR A TELA DE FALHA.
   A tela "Não foi possível concluir a abertura" aparecia por causa de falhas
   passageiras que se resolvem sozinhas em 1-2 segundos: janela fechada no meio
   do carregamento, wi-fi trocando para dados, um 429/5xx do Drive, um token
   ainda renovando. Agora esses casos são repetidos em silêncio (até 2 vezes,
   com espera curta) e a tela de falha só aparece quando o problema é real. */
let borionBootSilentRetries=0;
const BORION_BOOT_MAX_SILENT_RETRIES=2;
function borionBootErrorIsTransient642(error){
  const code=String(error&&error.code||'');
  if(['DRIVE_BOOT_PENDING','DRIVE_BOOT_CONFIRMATION','DEVICE_STORAGE_TIMEOUT','DURABLE_QUEUE_TIMEOUT','BOOT_TASK_TIMEOUT'].includes(code))return true;
  const message=String(error&&error.message||error||'').toLowerCase();
  if(/sem internet|offline/.test(message))return false;
  return /failed to fetch|networkerror|load failed|tempo limite|timeout|timed out|status 429|status 5\d\d|conexão|conexao|rede/.test(message);
}
function borionBootCanSilentlyReauth642(error){
  if(!navigator.onLine)return false;
  if(!(window.GoogleDriveAuth&&GoogleDriveAuth.hasUsableToken&&GoogleDriveAuth.hasUsableToken()))return false;
  return !!(window.GoogleDriveProvider&&GoogleDriveProvider._isAuthError&&GoogleDriveProvider._isAuthError(error));
}
async function runBorionBoot642(){
  ...
  const reconnect=async()=>{
    // A abertura recomeça do zero (contador de tentativas silenciosas incluso),
    // porque agora é a pessoa quem pediu explicitamente.
    borionBootSilentRetries=0;
    const interactive=!(window.GoogleDriveAuth&&GoogleDriveAuth.hasUsableToken&&GoogleDriveAuth.hasUsableToken());
  ...
  try{
    ...
    // Antes de qualquer tela de erro: uma tentativa silenciosa para falhas que
    // costumam se resolver sozinhas. A pasta vinculada e os dados nunca são
    // tocados aqui — é apenas uma releitura.
    const transient=borionBootErrorIsTransient642(error);
    const reauth=borionBootCanSilentlyReauth642(error);
    if((transient||reauth)&&borionBootSilentRetries<BORION_BOOT_MAX_SILENT_RETRIES&&navigator.onLine){
      borionBootSilentRetries+=1;
      const attempt=borionBootSilentRetries;
      console.warn(`[BORION][BOOT] falha passageira na abertura (tentativa silenciosa ${attempt}/${BORION_BOOT_MAX_SILENT_RETRIES}):`,error);
      if(window.BootProgress&&BootProgress.setDetail)BootProgress.setDetail('Concluindo a abertura...');
      borionBootRunning=false;
      await new Promise(resolve=>setTimeout(resolve,700*attempt));
      if(reauth){try{await GoogleDriveAuth.ensureFreshToken(false);}catch(refreshError){console.warn('[BORION][BOOT] renovação silenciosa do token não concluiu:',refreshError);}}
      return await runBorionBoot642();
    }
