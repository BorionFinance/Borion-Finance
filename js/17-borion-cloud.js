  ...
  renderCleanLogin(root){
    ...
    if(gd) gd.onclick=async()=>{
      ...
      try{ await GoogleDriveProvider.connect(true); }
        ...
        const isAuth=!!(window.GoogleDriveProvider&&GoogleDriveProvider._isAuthError&&GoogleDriveProvider._isAuthError(error));
        if(isAuth)this.returnToSimpleLogin(this.googleLoginError(error));
        else if(typeof renderGoogleDriveReconnect==='function')renderGoogleDriveReconnect(error&&error.message||String(error),{code:error&&error.code||'',preserveAuth:true});
        else{this.error='A conta Google foi confirmada, mas o Drive ainda não concluiu a abertura. Tente novamente.';this.render();}
      }
    };
