...
const OrderPreferences = {
  ...
  reconcile(type, currentIds){
    ...
    // Borion 7.0 — Notas entrou depois de muitos perfis já terem uma ordem do menu
    // salva. Nesses perfis, o reconciliador antigo colocava qualquer item novo no
    // fim, fazendo Notas aparecer depois de Configurações e, visualmente, abaixo do
    // Hub Borion. Na primeira inclusão, encaixamos Notas antes de Configurações.
    // Uma posição já salva pelo usuário continua sendo respeitada.
    if(type==='modules' && missing.includes('notes')){
      const settingsIndex=cleaned.indexOf('settings');
      if(settingsIndex>=0){
        cleaned.splice(settingsIndex,0,'notes');
        already.add('notes');
      }
    }

    currentIds.forEach(id=>{ if(!already.has(id)){ cleaned.push(id); already.add(id); } });
    return cleaned;
  },
  ...
  ensureBanner(){
    ...
    bar.innerHTML = `
      <div class="omb-text">
        ...
        <span>${isReservaGrid?'Escolha 2, 3 ou 4 colunas e arraste os Cofrinhos entre os slots.':(isPatrimonyGrid?'Escolha 1, 2 ou 3 colunas e arraste os módulos do Patrimônio.':'Segure a alça e arraste os itens para definir a ordem desejada.')}</span>
      </div>
      ...
      <div class="settings-section settings-hero-section order-organize-section">
        <div class="order-organize-head">
          <div>
            ...
            <p class="desc">Ative para reordenar os módulos do menu lateral e os itens de bancos, cartões e reservas — por arraste. Desativado, o Borion funciona normalmente e nada pode ser movido sem querer.</p>
          </div>
        ...
        ${active?`<div class="order-active-hint">Modo de organização ativo. Segure a alça e arraste os itens para definir a ordem desejada. Este modo também vale nas telas de Cartões e Contas e Reserva — use os botões no rodapé da tela para salvar, cancelar ou restaurar.</div>`:''}
...
function borionBindTransientDrag(pointerId,{onMove,onFinish,onCancel,captureTarget}){
  ...
  const lostCapture=ev=>finish(ev);
...
function borionOrderRows(container){return Array.from(container.querySelectorAll(':scope > [data-order-id]'));}
function borionOrderCaptureRects(container){const map=new Map();borionOrderRows(container).forEach(el=>map.set(el,el.getBoundingClientRect()));return map;}
function borionOrderAnimateFlip(container,before){
  if(!before||typeof Element==='undefined')return;
  borionOrderRows(container).forEach(el=>{
    const first=before.get(el);if(!first)return;
    const last=el.getBoundingClientRect(),dx=first.left-last.left,dy=first.top-last.top;
    if(Math.abs(dx)<1&&Math.abs(dy)<1)return;
    if(typeof el.animate==='function')el.animate([{transform:`translate(${dx}px,${dy}px)`},{transform:'translate(0,0)'}],{duration:210,easing:'cubic-bezier(.2,.85,.25,1)'});
    else{el.style.transform=`translate(${dx}px,${dy}px)`;el.classList.add('order-live-moving');requestAnimationFrame(()=>{el.style.transform='';setTimeout(()=>el.classList.remove('order-live-moving'),230);});}
  });
}
function borionOrderAutoScroll(y){
  const edge=72,speed=18;if(y<edge)window.scrollBy({top:-speed,behavior:'auto'});else if(y>window.innerHeight-edge)window.scrollBy({top:speed,behavior:'auto'});
}
function borionStartOrderDrag(handle,row,container,startEvent){
  const type=container.getAttribute('data-order-list');
  if(!type)return;
  const pointerId=startEvent.pointerId,rect=row.getBoundingClientRect();
  const offsetX=startEvent.clientX-rect.left,offsetY=startEvent.clientY-rect.top;
  const ghost=row.cloneNode(true);ghost.classList.add('order-drag-ghost');ghost.classList.remove('order-dragging');
  ghost.querySelectorAll('button,input,select,textarea,a').forEach(el=>{el.tabIndex=-1;el.setAttribute('aria-hidden','true');});
  Object.assign(ghost.style,{left:rect.left+'px',top:rect.top+'px',width:rect.width+'px',height:rect.height+'px'});
  document.body.appendChild(ghost);row.classList.add('order-dragging');container.classList.add('order-dragging-active');document.body.classList.add('borion-order-dragging');
  let target=null,cleaned=false,moved=false,lastX=startEvent.clientX,lastY=startEvent.clientY;
  function moveGhost(ev){lastX=ev.clientX;lastY=ev.clientY;ghost.style.left=(ev.clientX-offsetX)+'px';ghost.style.top=(ev.clientY-offsetY)+'px';}
  function clearTarget(){if(target)target.classList.remove('order-drop-target');target=null;}
  function choosePosition(next,ev){
    const r=next.getBoundingClientRect();
    const sameVisualRow=Math.abs(ev.clientY-(r.top+r.height/2))<Math.max(20,r.height*.42);
    const before=sameVisualRow?ev.clientX<(r.left+r.width/2):ev.clientY<(r.top+r.height/2);
    return before?'before':'after';
  }
  function onMove(ev){
    if(ev.cancelable)ev.preventDefault();moveGhost(ev);borionOrderAutoScroll(ev.clientY);
    if(Math.abs(ev.clientX-startEvent.clientX)>4||Math.abs(ev.clientY-startEvent.clientY)>4)moved=true;
    const candidates=borionOrderRows(container).filter(candidate=>candidate!==row);
    if(!candidates.length)return;
    const layout=getComputedStyle(container),horizontal=layout.display.includes('flex')&&!layout.flexDirection.startsWith('column');
    let reference=null,highlight=null;
    if(horizontal){
      for(const candidate of candidates){
        const r=candidate.getBoundingClientRect(),sameBand=ev.clientY>=r.top-6&&ev.clientY<=r.bottom+6;
        if(ev.clientY<r.top-6||(sameBand&&ev.clientX<r.left+r.width/2)){reference=candidate;highlight=candidate;break;}
      }
      if(!highlight)highlight=candidates[candidates.length-1];
    }else{
      for(const candidate of candidates){const r=candidate.getBoundingClientRect();if(ev.clientY<r.top+r.height/2){reference=candidate;highlight=candidate;break;}}
      if(!highlight)highlight=candidates[candidates.length-1];
    }
    const before=borionOrderCaptureRects(container);
    if(reference){if(row.nextElementSibling!==reference)container.insertBefore(row,reference);}
    else if(row!==container.lastElementChild)container.appendChild(row);
    borionOrderAnimateFlip(container,before);
    if(target!==highlight){clearTarget();target=highlight;if(target)target.classList.add('order-drop-target');}
  }
  function persist(){
    if(!container.isConnected)return;
    const ids=borionOrderRows(container).map(el=>el.getAttribute('data-order-id'));
    if(type==='budget_summary'&&window.Settings&&typeof Settings.summaryReorderFromDom==='function')Settings.summaryReorderFromDom(ids);
    else if(type==='financial_profiles'&&window.Settings&&typeof Settings.profilesReorderFromDom==='function')Settings.profilesReorderFromDom(ids);
    else OrderPreferences.reorderFromDom(type,ids);
  }
  function finish(){cleanup();if(moved)persist();}
  function cancel(){cleanup();}
  function cleanup(){
    if(cleaned)return;cleaned=true;clearTarget();row.classList.remove('order-dragging');container.classList.remove('order-dragging-active');document.body.classList.remove('borion-order-dragging');ghost.remove();
  }
  moveGhost(startEvent);
  borionBindTransientDrag(pointerId,{onMove,onFinish:finish,onCancel:cancel,captureTarget:null});
}
