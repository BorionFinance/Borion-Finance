...
function renderSmartphoneOverview(){
  ...
  const allTransactions=S.data.transacoes||[];
  const transactionOrder=new Map(allTransactions.map((item,index)=>[String(item&&item.id||''),index]));
  const recent=allTransactions
    .filter(t=>String(t.data||'').startsWith(monthPrefix) && bankMatches(t.banco,t.accountId))
    .slice()
    .sort((a,b)=>BudgetDateSort.compare('receita',a.data,b.data,a.addedAt||a.createdAt,b.addedAt||b.createdAt,transactionOrder.get(String(a.id)),transactionOrder.get(String(b.id))))
    .slice(0,6);
  ...
  return `<div class="smartphone-home">
    ...
    <section class="panel-box smart-recent-panel">
      <div class="toolbar"><div class="toolbar-left">Últimos lançamentos</div><button class="link-btn" onclick="SmartphoneMode.goBudget('receita')">Ver todos</button></div>
...
const SmartphoneMode={
    ...
    const box=el(`<div class="modal-overlay smart-launch-overlay">
      <div class="modal-box smart-launch-modal">
        ...
        <div class="smart-launch-grid">
          ...
          ${smartphoneQuickActionHTML('assinatura','◌','Assinatura','Cobrança recorrente')}
          ${reserveButton}
  ...
  launch(action){
    ...
    if(action==='assinatura'){
      S.view='budget'; S.budgetTab='assinaturas'; renderApp(); setTimeout(()=>Assinaturas.add(),60); return;
    }
    if(action==='reserva'){
