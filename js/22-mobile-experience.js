...
const MobileExperience = {
  ...
  decorateNotificationPanel(panel){
    if(!panel || !this.isSmart()) return;
    const handle=panel.querySelector('.notif-panel-handle');
    /* O conteúdo do painel é recriado quando uma notificação muda. A marca fica no handle,
       não no painel, para que o novo puxador nunca perca os listeners do gesto. */
    if(!handle || handle.dataset.mobileSheetGestures==='1') return;
    handle.dataset.mobileSheetGestures='1';
    panel.dataset.mobileSheetGestures='1';

    let pointerId=null,startY=0,lastY=0,lastAt=0,settleTimer=null;
    const clearSettle=()=>{if(settleTimer){clearTimeout(settleTimer);settleTimer=null;}};
    const settleBack=()=>{
      clearSettle();
      const current=panel.style.getPropertyValue('--notif-sheet-y')||'0px';
      panel.classList.remove('is-panel-dragging');
      panel.classList.add('is-panel-settling');
      panel.style.setProperty('--notif-sheet-y',current);
      void panel.offsetHeight;
      requestAnimationFrame(()=>{
        if(!panel.isConnected)return;
        panel.style.setProperty('--notif-sheet-y','0px');
        document.body.style.setProperty('--notif-overlay-opacity','1');
      });
      settleTimer=setTimeout(()=>{
        if(!panel.isConnected)return;
        panel.classList.remove('is-panel-settling');
    ...
    handle.addEventListener('pointermove',e=>{
      ...
      if(dy<=0)return;
      e.preventDefault();
      const resisted=dy/(1+dy/650);
      panel.style.setProperty('--notif-sheet-y',`${resisted}px`);
      document.body.style.setProperty('--notif-overlay-opacity',String(Math.max(.18,1-resisted/520)));
      lastY=e.clientY;
      lastAt=performance.now();
    },{passive:false});
    const finish=e=>{
      ...
      if(dy>82 || (velocity>.75&&dy>38)){
        this.haptic(7);
        panel.classList.remove('is-panel-dragging');
        if(window.Notifs||typeof Notifs!=='undefined')Notifs.closePanel({fromDrag:true});
        pointerId=null;
      }else settleBack();
    };
    handle.addEventListener('pointerup',finish,{passive:true});
    handle.addEventListener('pointercancel',settleBack,{passive:true});
  },
