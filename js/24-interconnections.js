(() => {
  ...
  function ensureSourceConfig(config){
    ...
    if(config.sourceAppId === 'marco-iris'){
      ...
      // Mantidos apenas para leitura/migração de bases antigas. A partir da v6.46.68,
      // despesas não dependem mais de categoria, conta, reserva ou revisão configurada.
    ...
    // v6.46.68 — fluxo único de despesas externas: entra como variável genérica,
    // sem conta e em aberto. O saldo só é movimentado quando o usuário completa a edição.
    config.expenseAutoImportEnabled = config.expenseAutoImportEnabled !== false;
    config.allowVariableToFixedTransition = config.allowVariableToFixedTransition === true;
    config.expenseGenericCategory = String(config.expenseGenericCategory || 'Despesa integrada').trim() || 'Despesa integrada';
  ...
  function openHandleDb(){
    return Promise.reject(new Error('Integrações por pasta local foram desativadas.'));
  }
  async function handleTx(mode, key, value){
    throw new Error('Integrações por pasta local foram desativadas.');
  }
  ...
  async function readLocalSnapshot(row){
    throw new Error('Integrações por pasta local foram desativadas. Use o Google Drive.');
  }
  ...
  async function readSnapshot(row){
    if(row.config.transport !== 'drive') throw new Error('Esta integração antiga usava uma pasta local. Reconecte-a pelo Google Drive para continuar.');
    return readDriveSnapshot(row);
  }
  ...
  function existingAckResults(data,state,snapshot){
    ...
    return (snapshot.records||[]).map(record=>{
      ...
      else if(marker?.status==='waiting'||pendingKeys.has(key)||pendingKeys.has(String(record.aggregateId))||pendingKeys.has(String(record.entityId)))result=record.direction==='expense'?'paused_expense':'already_processed';
      else if(record.active===false||normalize(record.status).includes('cancel'))result='cancelled';
      return {sourceRecordId:key,aggregateId:record.aggregateId,entityId:record.entityId,result,status:result,processedRevision:Number(snapshot.revision)||0,borionId,borionTransactionId:borionId,processedAt:nowIso(),message:result==='paused_expense'?'A despesa permanece pendente porque a importação automática está pausada.':'Registro já processado; nenhuma duplicidade criada.'};
    });
  }
  ...
  function findImportedNativeItems(data,sourceAppId,aggregateId,entityId='',entityKind=''){
    const source=String(sourceAppId||''),aggregate=String(aggregateId||''),entity=String(entityId||''),kind=String(entityKind||'');
    const refs=[];
    const collect=(item,ref)=>{if(item&&String(item.integrationSourceAppId||'')===source)refs.push(Object.assign({item},ref));};
    (data.transacoes||[]).forEach(item=>collect(item,{kind:'transacoes'}));
    (data.fixas||[]).forEach(item=>collect(item,{kind:'fixas'}));
    (data.cartoes||[]).forEach(card=>(card?.parcelas||[]).forEach(item=>collect(item,{kind:'card-parcel',card})));

    // O aggregateId identifica o registro externo completo e tem precedência absoluta.
    // Não misture o fallback por entityId quando houver correspondência agregada: receitas
    // e despesas do mesmo aplicativo podem legitimamente reutilizar o mesmo ID numérico.
    if(aggregate){
      const exact=refs.filter(ref=>String(ref.item.integrationAggregateId||'')===aggregate);
      if(exact.length)return exact;
    }
    if(!entity)return [];
    const entityMatches=item=>{
      if(kind==='expense')return String(item.integrationExpenseId||'')===entity||(!item.integrationReceiptId&&String(item.integrationEntityId||'')===entity);
      if(kind==='receipt')return String(item.integrationReceiptId||'')===entity||(!item.integrationExpenseId&&String(item.integrationEntityId||'')===entity);
      return [item.integrationExpenseId,item.integrationReceiptId,item.integrationEntityId].map(value=>String(value||'')).includes(entity);
    };
    return refs.filter(ref=>entityMatches(ref.item));
  }
  function findImportedNativeItem(data,sourceAppId,aggregateId,entityId='',entityKind=''){
    return findImportedNativeItems(data,sourceAppId,aggregateId,entityId,entityKind)[0]||null;
  }
  function removeImportedFixedFromBorion(data,fixed,reason='integration_source_deleted'){
    ...
    payments.forEach(payment=>{
      if(payment.pago){
        if(payment.origemPagamento==='reserva'&&payment.reservaMoveId){
          const move=data.reservas.moves.find(item=>item&&item.id===payment.reservaMoveId);
          const box=reserveByIdIn(data,payment.reservaId||move?.boxId);
          const value=Number(payment.valorPago)||Number(move?.valor)||Number(fixed.valor)||0;
          if(box)box.valorAtual=Math.round(((Number(box.valorAtual)||0)+value)*100)/100;
          data.reservas.moves=data.reservas.moves.filter(item=>item&&item.id!==payment.reservaMoveId);
          recordLocalDeletion(data,['reservas','moves'],payment.reservaMoveId,reason);
        }else if(payment.origemPagamento==='conta'){
          adjust(data,payment.accountId||fixed.accountId,Number(payment.valorPago)||Number(fixed.valor)||0);
        }
      }
    ...
    recordLocalDeletion(data,'fixas',fixed.id,reason);
    return true;
  }

  function removeImportedNativeGroup(sourceItem,data=S.data,reason='integration_source_deleted'){
    const sourceAppId=String(sourceItem?.integrationSourceAppId||''),aggregateId=String(sourceItem?.integrationAggregateId||''),entityId=String(sourceItem?.integrationExpenseId||sourceItem?.integrationReceiptId||sourceItem?.integrationEntityId||'');
    if(!sourceAppId||(!aggregateId&&!entityId))return false;
    const entityKind=sourceItem?.integrationExpenseId?'expense':(sourceItem?.integrationReceiptId?'receipt':'');
    const refs=findImportedNativeItems(data,sourceAppId,aggregateId,entityId,entityKind);
    const financialRefs=refs.filter(ref=>ref.kind!=='card-parcel');
    financialRefs.forEach(ref=>{
      if(ref.kind==='fixas')removeImportedFixedFromBorion(data,ref.item,reason);
      else{
        const reserveMoveIds=[ref.item.reservaMoveId,ref.item.reservaOrigemMoveId].filter(Boolean);
        reverseImportedTransaction(data,ref.item);
        recordLocalDeletion(data,'transacoes',ref.item.id,reason);
        reserveMoveIds.forEach(moveId=>recordLocalDeletion(data,['reservas','moves'],moveId,reason));
      }
    });
    refs.filter(ref=>ref.kind==='card-parcel').forEach(ref=>{
      ref.card.parcelas=(ref.card.parcelas||[]).filter(item=>item!==ref.item&&String(item.id)!==String(ref.item.id));
      ref.card.updatedAt=Date.now();
    });
  ...
  function removeMitExpenseFromBorion(data,state,mitState,entityId,aggregateId,deletedAt,reason='source_cancelled'){
    const expenseId=String(entityId||''),markerEntry=findMitMarkerByEntity(state,expenseId),expenseState=mitState.expenses[expenseId]||{};
    const officialAggregate=String(aggregateId||expenseState.aggregateId||markerEntry?.aggregateId||'');
    const refs=findImportedNativeItems(data,'marco-iris',officialAggregate,expenseId,'expense');
    const sourceItem=refs[0]?.item||null;
    const borionId=String(sourceItem?.id||expenseState.borionId||markerEntry?.marker?.txId||'');
    const removed=sourceItem?removeImportedNativeGroup(sourceItem,data,reason):false;
  ...
  /* v6.46.68 — DESPESA EXTERNA GENÉRICA
     Toda despesa recebida do Marco Iris ou Amanda entra imediatamente como despesa
     variável nativa, em aberto e sem conta/reserva/cartão. Isso elimina a fila de
     revisão e impede qualquer débito automático antes de o usuário completar os dados. */
  function makeGenericIntegratedExpense(data,config,record){
    const sourceAppId=String(config?.sourceAppId||record?.sourceAppId||'').trim();
    const sourceLabel=sourceName(sourceAppId);
    const category=ensureCategory(data,'variavel',String(config?.expenseGenericCategory||'Despesa integrada'));
    const value=Math.round((Number(record?.amount)||0)*100)/100;
    if(value<=0) throw new Error('Valor inválido para importar a despesa integrada.');
    const aggregateId=String(record?.aggregateId||'').trim();
    const entityId=String(record?.entityId||record?.expenseId||'').trim();
    const createdAt=Date.now();
    const tx={
      id:'bridge-expense-'+hash(aggregateId||`${sourceAppId}:${entityId}:${createdAt}`),
      tipo:'variavel',
      nome:String(record?.name||record?.description||`Despesa de ${sourceLabel}`).trim()||`Despesa de ${sourceLabel}`,
      localCompra:String(record?.localPurchase||'').trim(),
      categoria:category,
      valor:value,
      data:record?.date||record?.dueDate||todayISO(),
      statusPagamento:'Em aberto',
      accountId:null,banco:'',origemPagamento:null,formaPagamento:null,
      reservaOrigemId:null,reservaOrigemMoveId:null,
      createdAt,addedAt:createdAt,updatedAt:createdAt,
      integrationImported:true,integrationManaged:false,integrationImportMode:'native-generic-expense',
      integrationAggregateId:aggregateId,integrationSourceAppId:sourceAppId,integrationEntityId:entityId,
      integrationExpenseId:entityId,integrationImportedAt:nowIso(),
      ...
      integrationOriginalExpenseType:record?.expenseType||'',
      integrationOriginalCategory:record?.category||'',
      integrationOriginalSourceValues:clone(sourceBag(record||{})),integrationMappingVersion:SPEC.mappingVersion,
      integrationNeedsCompletion:true,integrationCompletionStatus:'pending',
      integrationCompletionReason:'missing_financial_destination',integrationGenericExpense:true
    };
    tx.integrationOriginalFingerprint=record?.fingerprint||hash(record||{});
    tx.integrationEditGuardHash=editableSnapshotHash(tx);
    return tx;
  }
  function commitMitExpenseAuto(data,config,record){
    const tx=makeGenericIntegratedExpense(data,config,record);
    data.transacoes=Array.isArray(data.transacoes)?data.transacoes:[];
    if(data.transacoes.some(item=>item.id===tx.id)) tx.id+='-'+hash(nowIso()+Math.random());
  ...
  function reconcileMitSnapshotMutable(data,config,snapshot,{mode='manual'}={}){
    ...
    const hasDeferredExpense=(interop.pending||[]).some(item=>item?.sourceAppId==='marco-iris')||Object.values(state.records||{}).some(marker=>marker?.status==='waiting'&&['manual_review','expense_auto_paused'].includes(marker?.reason));
    if(incomingRevision===Number(state.lastRevision||0)&&incomingHash===state.lastContentHash&&!hasDeferredExpense){
      const results=existingAckResults(data,state,snapshot);
      return {results,pending:[],summary:Object.assign({},config.lastResult||{},{processed:results.length,created:0,createdReceipts:0,createdExpenses:0,deleted:0,unchanged:results.length,pendingExpenses:0,errors:0}),changed:false};
    }
    state.records ||= {};
    const results=[];
    ...
    (snapshot.records||[]).forEach(record=>{
      ...
      if(record.direction==='expense'){
        ...
        deletePending(record);
        if(config.expenseAutoImportEnabled===false){
          state.records[aggregateId]={status:'waiting',entityId,sourceRecordId:recordKey,reason:'expense_auto_paused',fingerprint:record.fingerprint||hash(record),updatedAt:record.sourceUpdatedAt||snapshot.generatedAt||nowIso()};
          results.push({sourceRecordId:recordKey,aggregateId,entityId,status:'paused_expense',result:'paused',borionTransactionId:'',message:'Importação automática de despesas pausada neste perfil.'});
          return;
        }
        const borionId=commitMitExpenseAuto(data,config,record),importedAt=nowIso();
        mitState.expenses[entityId]={borionId,aggregateId,importedAt};
        state.records[aggregateId]={status:'imported',txId:borionId,entityId,sourceRecordId:recordKey,importedAt,fingerprint:record.fingerprint||hash(record)};
        results.push({sourceRecordId:recordKey,aggregateId,entityId,status:'expense_created',result:'imported',borionTransactionId:borionId,message:'Despesa importada como variável genérica, em aberto e sem conta.'});
        return;
      }
    ...
    // A fila manual antiga é encerrada definitivamente. Despesas pausadas ficam
    // apenas como marcadores técnicos e entram quando a automação for reativada.
    pendingById.clear();
    interop.pending=(interop.pending||[]).filter(item=>item.sourceAppId!=='marco-iris');
    ...
    config.lastResult={processed:results.length,created:results.filter(x=>x.status==='created').length,createdReceipts:results.filter(x=>x.status==='created').length,createdExpenses:results.filter(x=>x.status==='expense_created').length,deleted:results.filter(x=>x.status==='cancelled').length,
      waiting:results.filter(x=>x.status==='waiting_receipt').length,waitingReceipts:results.filter(x=>x.status==='waiting_receipt').length,pendingExpenses:0,pausedExpenses:results.filter(x=>x.status==='paused_expense').length,unchanged:results.filter(x=>x.status==='unchanged').length,
    ...
    return {results,pending:[],summary:config.lastResult,changed:true};
  }
  ...
  function reconcileSnapshot(data, config, snapshot, options={}){
    ...
    const sourceTombstones = new Map((snapshot.tombstones || []).map(item => [item.aggregateId, item]));
    const tombstones = new Set(sourceTombstones.keys());

    // V6.46.67 — o arquivo da Amanda é um snapshot completo. Quando ele vem da
    // mesma instalação já vinculada e um registro anteriormente importado não
    // existe mais, a ausência também é uma exclusão confiável. Isso repara bases
    // antigas em que o shadow/tombstone da ponte se perdeu antes desta correção.
    const trustedAmandaSnapshot = config.sourceAppId === 'amanda-estetica' &&
      snapshot.completeSnapshot === true && !!snapshot.instanceId &&
      Number(snapshot.revision || 0) >= Number(state.lastRevision || 0);
    const sameAmandaInstance = trustedAmandaSnapshot && !!state.instanceId &&
      String(state.instanceId) === String(snapshot.instanceId);
    if(sameAmandaInstance){
      Object.entries(state.records).forEach(([aggregateId, marker]) => {
        if(marker?.status === 'imported' && !incomingIds.has(aggregateId) && !tombstones.has(aggregateId)){
          tombstones.add(aggregateId);
        }
      });
    }
    if(trustedAmandaSnapshot){
      const expectedPrefix = `amanda-estetica:${snapshot.instanceId}:`;
      const nativeItems=[...(data.transacoes||[]),...(data.fixas||[])];
      (data.cartoes||[]).forEach(card=>nativeItems.push(...(card?.parcelas||[])));
      nativeItems.forEach(item => {
        const aggregateId=String(item?.integrationAggregateId||'');
        if(item?.integrationSourceAppId==='amanda-estetica'&&aggregateId.startsWith(expectedPrefix)&&
          !incomingIds.has(aggregateId)&&!tombstones.has(aggregateId))tombstones.add(aggregateId);
      });
    }
    ...
    tombstones.forEach(aggregateId => {
      ...
      const sourceDeletion = sourceTombstones.get(aggregateId) || null;
      const deletedAt = sourceDeletion?.deletedAt || nowIso();
      const nativeRef=findImportedNativeItem(data,config.sourceAppId,aggregateId);
      const nativeTx=nativeRef?.item||null;

      if(!nativeTx){
        state.records[aggregateId] = {
          status:'source_deleted', txId:'', previousTxId:marker?.txId || marker?.previousTxId || '',
          entityId:marker?.entityId || '', deletedAt,
          sourceRevision:Number(snapshot.revision)||0, reason:sourceDeletion?.reason || 'source_snapshot_absent'
        };
        results.push({
      ...
      const nativeRefs=findImportedNativeItems(data,config.sourceAppId,aggregateId,marker?.entityId||'');
      const editedSinceImport=nativeRefs.filter(ref=>ref.kind!=='card-parcel').some(ref=>!!ref.item.integrationEditGuardHash&&ref.item.integrationEditGuardHash!==editableSnapshotHash(ref.item));
      // Despesas integradas obedecem à origem mesmo depois de completadas, convertidas
      // para fixa ou editadas no Borion. Receitas mantêm a proteção histórica contra
      // exclusão automática quando foram alteradas manualmente.
      const isIntegratedExpense=nativeRefs.some(ref=>ref.item.integrationExpenseId||ref.item.integrationGenericExpense===true);
      if(deletionPolicy === 'delete' && (isIntegratedExpense || !editedSinceImport)){
        try{
          removeImportedNativeGroup(nativeTx,data,'integration_source_deleted');
          state.records[aggregateId] = {
            status:'source_deleted', txId:'', previousTxId:nativeTx.id,
            entityId:marker?.entityId || nativeTx.integrationEntityId || '', deletedAt,
            sourceRevision:Number(snapshot.revision)||0, reason:sourceDeletion?.reason || 'source_snapshot_absent'
          };
          results.push({
    ...
    snapshot.records.forEach(record => {
      ...
      let marker = state.records[record.aggregateId];
      if(marker?.status === 'source_deleted'){
        const sourceRevision = Number(marker.sourceRevision)||0;
        const deletedAtMs = Date.parse(marker.deletedAt||'')||0;
        const sourceUpdatedAtMs = Date.parse(record.sourceUpdatedAt||'')||0;
        const isExplicitNewRecreation = Number(snapshot.revision||0) > sourceRevision &&
          sourceUpdatedAtMs > deletedAtMs;
        if(!isExplicitNewRecreation){
          results.push({
            aggregateId:record.aggregateId, entityId:record.entityId, status:'source_deleted',
            borionTransactionId:'', message:'Reaparecimento antigo ignorado; a exclusão já foi confirmada.'
          });
          return;
        }
        delete state.records[record.aggregateId];
        marker = null;
      }
      const nativeRef=findImportedNativeItem(data,config.sourceAppId,record.aggregateId);
      const nativeTx=nativeRef?.item||null;
      ...
      if(record.direction==='expense'){
        if(config.expenseAutoImportEnabled===false){
          state.records[record.aggregateId]={status:'waiting',entityId:record.entityId,reason:'expense_auto_paused',updatedAt:record.sourceUpdatedAt||''};
          results.push({aggregateId:record.aggregateId,entityId:record.entityId,status:'paused_expense',borionTransactionId:'',message:'Importação automática de despesas pausada neste perfil.'});
          return;
        }
        const tx=makeGenericIntegratedExpense(data,config,record);
        data.transacoes=Array.isArray(data.transacoes)?data.transacoes:[];
        if(data.transacoes.some(item=>item.id===tx.id))tx.id+='-'+hash(nowIso()+Math.random());
        data.transacoes.push(tx);
        state.records[record.aggregateId]={fingerprint:tx.integrationOriginalFingerprint,status:'imported',txId:tx.id,entityId:record.entityId,importedAt:tx.integrationImportedAt,updatedAt:record.sourceUpdatedAt||''};
        results.push({aggregateId:record.aggregateId,entityId:record.entityId,status:'expense_created',borionTransactionId:tx.id,message:'Despesa importada como variável genérica, em aberto e sem conta.'});
        return;
      }
    ...
    config.lastResult = {
      ...
      createdExpenses:results.filter(x => x.status === 'expense_created').length,
      deleted:results.filter(x => x.status === 'deleted').length,
      waiting:results.filter(x => x.status === 'waiting').length,
      pausedExpenses:results.filter(x => x.status === 'paused_expense').length,
  ...
  async function syncSource(sourceAppId, {silent=false,button=null}={}){
    ...
    if(!row.config.mappingReady) throw new Error(sourceAppId==='marco-iris'?'Abra “Configurar receitas e despesas”, configure somente as receitas e clique em “Salvar opções”.':'Abra a aba Vínculos, configure as receitas e clique em “Salvar opções”.');
    ...
    try{
      ...
      if(liveRow.config.transport!=='drive') throw new Error('A confirmação local foi desativada. Reconecte a integração pelo Google Drive.');
      await writeDriveAck(liveRow,ack);
      ...
      if(!silent&&typeof toast==='function'){
        ...
        const expenseCreatedText=summary.createdExpenses?`, ${Number(summary.createdExpenses)} despesa(s) genérica(s) nova(s)`:'';
        const pausedText=summary.pausedExpenses?`, ${Number(summary.pausedExpenses)} despesa(s) pausada(s)`:'';
        const deletedText=summary.deleted?`, ${Number(summary.deleted)} lançamento(s) excluído(s)`:'';
        toast(`${sourceName(sourceAppId)}: ${Number(summary.created||0)} receita(s) nova(s)${expenseCreatedText}${pausedText}${deletedText}, ${Number(summary.unchanged||0)} já processada(s).`);
      }
      return result;
    }catch(error){
      if(error?.code==='INSTANCE_CONFLICT'){
        try{
          ...
          if(row.config.transport!=='drive')throw new Error('A confirmação local foi desativada. Reconecte a integração pelo Google Drive.');
          await writeDriveAck(row,rejected);
        }catch(_ackError){}
      }
  ...
  async function configure(sourceAppId, transport, profileId, accountId){
    ...
    if(transport!=='drive') throw new Error('Por segurança, as integrações usam somente o Google Drive.');
  ...
  const MAPPING_HELP = Object.freeze({
    ...
    mitExpenseRules:{
      title:'Fluxo genérico de despesas integradas',
      summary:'Toda despesa recebida entra direto em Despesa variável, em aberto e sem conta. Nenhum saldo é movimentado antes da edição no Borion.',
      steps:['Mantenha a importação automática ativada ou pause quando não quiser trazer novas despesas.','A despesa aparece em Lançamentos → Despesa variável com o status Completar.','Edite, escolha conta, reserva, carteira ou cartão e confirme o status.','Ative a transição variável → fixa somente nos perfis que realmente precisam dessa opção.'],
      example:['Compra recebida → Despesa variável genérica → Editar → Conta Nubank → Pago']
    },
    mitExpenses:{
      title:'Sem fila de revisão',
      summary:'A fila Aguardando Revisão foi removida. Despesas entram direto em Lançamentos para serem completadas no próprio editor nativo.',
      steps:['Abra Lançamentos → Despesa variável.','Procure o status Completar.','Edite os dados financeiros e salve.'],
      example:['Despesa do Marco Iris → Completar → editar normalmente no Borion']
    }
  });
  ...
  function incompleteIntegratedExpenseCount(data,sourceAppId){
    return (data?.transacoes||[]).filter(tx=>tx?.tipo==='variavel'&&tx.integrationSourceAppId===sourceAppId&&tx.integrationNeedsCompletion===true).length;
  }
  function expenseFlowSettingsHTML(sourceAppId,row){
    if(!row)return '';
    const config=ensureSourceConfig(row.config),enabled=config.expenseAutoImportEnabled!==false,allowFixed=config.allowVariableToFixedTransition===true;
    const incomplete=incompleteIntegratedExpenseCount(row.data,sourceAppId);
    return `<section class="interop-map-section interop-generic-expense-flow"><div class="mit-pending-head"><div>${interopSectionHeading('Fluxo das despesas integradas','mitExpenseRules','fluxo genérico das despesas')}<p>Toda despesa recebida entra diretamente em <b>Despesa variável</b>, com a categoria <b>${escHtml(config.expenseGenericCategory)}</b>, status <b>Em aberto</b> e <b>sem conta</b>. Ela não movimenta saldo de conta, reserva ou cartão até ser editada e salva no Borion.</p></div><span class="pill ${enabled?'ok':'warn'}">${enabled?'Importação ativa':'Importação pausada'}</span></div><div class="interop-rule-grid interop-expense-flow-grid"><div><b>Importar despesas</b><span>${enabled?'Novas despesas entram automaticamente como genéricas.':'Novas despesas ficam na origem e serão trazidas quando reativar.'}</span><div class="interop-actions"><button type="button" class="btn ${enabled?'btn-primary':'btn-outline'} btn-sm" onclick="BorionInterop.setExpenseFlowSetting('${escHtml(sourceAppId)}','auto',true,{button:this})">Ativar</button><button type="button" class="btn ${enabled?'btn-outline':'btn-primary'} btn-sm" onclick="BorionInterop.setExpenseFlowSetting('${escHtml(sourceAppId)}','auto',false,{button:this})">Pausar</button></div></div><div><b>Transição variável → fixa</b><span>${allowFixed?'O editor das despesas importadas mostra a opção Despesa fixa.':'A opção fica oculta para não alterar a usabilidade normal do perfil.'}</span><div class="interop-actions"><button type="button" class="btn ${allowFixed?'btn-primary':'btn-outline'} btn-sm" onclick="BorionInterop.setExpenseFlowSetting('${escHtml(sourceAppId)}','fixed',true,{button:this})">Habilitar</button><button type="button" class="btn ${allowFixed?'btn-outline':'btn-primary'} btn-sm" onclick="BorionInterop.setExpenseFlowSetting('${escHtml(sourceAppId)}','fixed',false,{button:this})">Desabilitar</button></div></div></div><div class="info-box"><b>${incomplete} despesa(s) para completar.</b> Abra Lançamentos → Despesa variável e edite as que estiverem com o status <b>Completar</b>.</div></section>`;
  }
  async function setExpenseFlowSetting(sourceAppId,key,value,{button=null}={}){
    const row=findSourceConfig(sourceAppId);if(!row)throw new Error('Integração não configurada neste perfil.');
    setAsyncButtonState(button,true,'Salvando…');
    try{
      ensureSourceConfig(row.config);
      if(key==='auto')row.config.expenseAutoImportEnabled=value===true;
      else if(key==='fixed')row.config.allowVariableToFixedTransition=value===true;
      else throw new Error('Configuração de despesas inválida.');
      saveProfileData(row.profile.id,row.data,{deferGoogleCommit:true,source:'interop_expense_flow_setting'});
      const confirmation=await persistCriticalChange('interop_expense_flow_setting');
      if(window.GoogleDriveProvider&&GoogleDriveProvider._isStrictMode?.()&&!confirmation.synced)return false;
      if(typeof toast==='function')toast(key==='auto'?(value?'Importação automática de despesas ativada. Sincronize para trazer as pendentes.':'Importação automática de despesas pausada.'):(value?'Transição de despesa variável para fixa habilitada neste perfil.':'Transição de despesa variável para fixa desabilitada neste perfil.'));
      if(typeof renderView==='function')renderView();
      return true;
    }finally{setAsyncButtonState(button,false);}
  }
  ...
  function renderConnectionTab(sourceAppId, row){
    const source = SOURCES[sourceAppId];
    if(!row){
      const intro=sourceAppId==='marco-iris'?'Conecte a pasta gerada pelo Marco Iris Tecnologia. Depois configure apenas categorias e destinos das receitas; despesas entram como variáveis genéricas.':'Conecte a pasta gerada pelo aplicativo. Configure as receitas; despesas entram como variáveis genéricas, em aberto e sem conta.';
      return `<div class="interop-pane"><div class="interop-empty"><div class="interop-empty-icon">⇄</div><h4>${escHtml(source.name)} ainda não está conectado</h4><p>${intro}</p><div class="interop-actions"><button class="btn btn-primary btn-sm" onclick="BorionInterop.setupDialog('${sourceAppId}','drive')">Conectar Google Drive</button></div></div></div>`;
    }
    const c = row.config;
    if(sourceAppId==='marco-iris'){
      ...
      return `<div class="interop-pane"><div class="interop-status-grid mit-connection-status"><div><span>Perfil de destino</span><strong>${escHtml(row.profile.name)}</strong></div><div><span>Conta padrão</span><strong>${escHtml(accountName(row.data,c.accountId)||'Não definida')}</strong></div><div><span>Meio</span><strong>Google Drive</strong></div><div><span>Última sincronização</span><strong>${escHtml(dateText(c.lastSyncAt))}</strong></div><div><span>Quantidade de registros processados</span><strong>${processed}</strong></div><div><span>Status da automação</span><strong><span class="pill ${statusClass}">${statusLabel}</span></strong></div></div>${reconnectBox}<div class="gold-box interop-sync-box"><b>Resumo da última sincronização</b><br><span>${Number(r.created||0)} receita(s) nova(s) · ${Number(r.createdExpenses||0)} despesa(s) genérica(s) nova(s) · ${Number(r.unchanged||0)} já processado(s) · ${Number(r.pausedExpenses||0)} despesa(s) pausada(s) · ${Number((r.ignoredBeforeCutoff??r.ignored)||0)} ignorado(s) pelo corte · ${Number(r.errors||0)} erro(s)</span>${c.lastError?`<br><b>Erro:</b> ${escHtml(c.lastError)}`:''}</div><div class="info-box"><b>Regra ativa:</b> cada pagamento Pago com Data de Pagamento entra individualmente; a OSV não precisa estar totalmente quitada. Toda despesa entra em Despesa variável como genérica, em aberto e sem conta, sem movimentar saldo até ser editada.</div><div class="interop-actions"><button class="btn btn-primary btn-sm" onclick="BorionInterop.syncSource('marco-iris',{button:this})" ${syncDisabled?`disabled title="${syncTitle}"`:''}>Sincronizar agora</button><button class="btn-outline btn-sm" onclick="BorionInterop.setupDialog('marco-iris','drive')">Reconfigurar conexão</button><button class="btn-outline btn-sm" onclick="BorionInterop.disconnect('marco-iris')">Desconectar</button></div>${!c.mappingReady?'<div class="interop-next-step"><b>Próximo passo:</b> abra a aba <b>Configurar receitas e despesas</b>, configure as receitas e clique em <b>Salvar opções</b>.</div>':''}</div>`;
    }
    ...
    return `<div class="interop-pane"><div class="interop-status-grid"><div><span>Perfil de destino</span><strong>${escHtml(row.profile.name)}</strong></div><div><span>Conta padrão</span><strong>${escHtml(accountName(row.data, c.accountId) || 'Carteira')}</strong></div><div><span>Meio</span><strong>Google Drive</strong></div><div><span>Mapeamento</span><strong>${mappingLabel}</strong></div></div><div class="gold-box interop-sync-box"><b>Última sincronização:</b> ${escHtml(dateText(c.lastSyncAt))}<br><span>${Number(r.created || 0)} novo(s) · ${Number(r.deleted || 0)} excluído(s) · ${Number(r.unchanged || 0)} já importado(s) · ${Number(r.waiting || 0)} aguardando · ${Number(r.ignored || 0)} ignorado(s)</span>${c.lastError ? `<br><b>Erro:</b> ${escHtml(c.lastError)}` : ''}</div><label class="interop-deletion-toggle"><input type="checkbox" ${deletionOn ? 'checked' : ''} onchange="BorionInterop.setDeletionPolicy('${sourceAppId}', this.checked ? 'delete' : 'preserve')"><span><b>Excluir aqui automaticamente</b> quando o lançamento for removido na origem<br><small>Despesas integradas seguem a exclusão da origem mesmo depois de completadas ou convertidas para fixa. Receitas editadas manualmente continuam protegidas.</small></span></label><div class="interop-actions"><button class="btn btn-primary btn-sm" onclick="BorionInterop.syncSource('${sourceAppId}',{button:this})" ${c.mappingReady ? '' : 'disabled title="Configure os Vínculos primeiro"'}>Sincronizar agora</button><button class="btn-outline btn-sm" onclick="BorionInterop.inspectSource('${sourceAppId}',{button:this})">Ler campos da origem</button><button class="btn-outline btn-sm" onclick="BorionInterop.setupDialog('${sourceAppId}','drive')">Reconfigurar conexão</button><button class="btn-outline btn-sm" onclick="BorionInterop.disconnect('${sourceAppId}')">Desconectar</button></div>${!c.mappingReady ? '<div class="interop-next-step"><b>Próximo passo:</b> abra a aba <b>Vínculos</b>, confira as conversões e salve. Só depois os lançamentos serão importados.</div>' : ''}</div>`;
  }
  ...
  function renderLinksTab(sourceAppId, row){
    ...
    if(!hasFields) return `<div class="interop-pane interop-links-pane">${expenseFlowSettingsHTML(sourceAppId,row)}<div class="interop-empty"><h4>Nenhum dado de receita foi lido</h4><p>O fluxo das despesas acima já pode ser configurado. Use o botão abaixo para analisar as receitas da origem sem importar lançamentos.</p><button class="btn btn-primary btn-sm" onclick="BorionInterop.inspectSource('${sourceAppId}',{button:this})">Ler dados da origem</button></div></div>`;
    const sourceDisplay=item=>`<strong>${escHtml(friendlySourceValue(item.label||item.value,item.field))}</strong>`;
    const directionRows=(d.directions.length?d.directions:['income']).filter(direction=>direction==='income').map(direction=>{
    ...
    const kindRows=d.transactionKinds.filter(item=>item.direction==='income').map(item=>{
    ...
    const categoryRows=d.categories.filter(item=>item.direction==='income').map(item=>{
    ...
    const paymentRows=d.paymentMethods.filter(item=>item.direction==='income').map(item=>{
    ...
    const statusRows=d.statuses.filter(item=>item.direction==='income').map(item=>{
  ...
  function renderSourceWorkspace(sourceAppId){
    const source = SOURCES[sourceAppId];
    const row = findSourceConfig(sourceAppId);
    const linksLabel=sourceAppId==='marco-iris'?'Configurar receitas e despesas':'Vínculos';
  ...
  function renderSettings(){
    ...
    return `<div class="settings-page interop-settings-page"><div class="settings-section settings-hero-section"><h3>Integrações inteligentes</h3><p class="desc">Receitas seguem os destinos configurados. Despesas do Marco Iris e da Amanda entram automaticamente como variáveis genéricas, em aberto e sem conta.</p></div><div class="interop-app-tabs">${Object.entries(SOURCES).map(([id, source]) => {
      const row = findSourceConfig(id);
      return `<button class="interop-app-tab ${uiSourceAppId === id ? 'active' : ''}" onclick="BorionInterop.setSettingsSource('${id}')"><span>${escHtml(source.name)}</span><small>${row ? (row.config.mappingReady ? 'Conectado e configurado' : (id==='marco-iris'?'Conectado · configurar receitas':'Conectado · configurar receitas')) : 'Não conectado'}</small></button>`;
    }).join('')}</div>${renderSourceWorkspace(uiSourceAppId)}<div class="settings-section interop-rules-summary"><h3>Como a sincronização funciona</h3><div class="interop-rule-grid"><div><b>1. Identifica</b><span>Confere o ID permanente do registro externo.</span></div><div><b>2. Converte</b><span>Receitas usam os vínculos; despesas entram genéricas e sem movimentar saldo.</span></div><div><b>3. Torna nativo</b><span>O lançamento pode ser editado livremente no Borion.</span></div><div><b>4. Não duplica</b><span>O ID continua registrado mesmo após qualquer edição local.</span></div></div></div></div>`;
  }
  ...
  function importedSourceConfig(tx,data=S.data){
    const sourceAppId=String(tx?.integrationSourceAppId||'');
    if(!sourceAppId||!data)return null;
    const config=ensureInterop(data).sources?.[sourceAppId];
    return config?ensureSourceConfig(config):null;
  }
  function canTransitionExpense(tx,data=S.data){
    return !!(tx&&tx.tipo==='variavel'&&tx.integrationAggregateId&&tx.integrationNeedsCompletion===true&&importedSourceConfig(tx,data)?.allowVariableToFixedTransition===true);
  }
  function updateImportedTargetReference(sourceTx,target,data=S.data){
    if(!sourceTx?.integrationAggregateId||!sourceTx?.integrationSourceAppId||!target)return false;
    const state=importedState(data,sourceTx.integrationSourceAppId),aggregateId=String(sourceTx.integrationAggregateId);
    const marker=state.records[aggregateId]||{};
    state.records[aggregateId]=Object.assign({},marker,{status:'imported',txId:target.id,entityId:sourceTx.integrationExpenseId||sourceTx.integrationEntityId||marker.entityId||'',importedAt:marker.importedAt||sourceTx.integrationImportedAt||nowIso()});
    if(sourceTx.integrationSourceAppId==='marco-iris'){
      const entityId=String(sourceTx.integrationExpenseId||sourceTx.integrationEntityId||'');
      if(entityId){const mitState=ensureMitState(data);mitState.expenses[entityId]=Object.assign({},mitState.expenses[entityId]||{},{borionId:target.id,aggregateId,importedAt:sourceTx.integrationImportedAt||nowIso(),status:'imported'});}
    }
    return true;
  }
  function completeImportedExpense(tx,targetType='variavel',data=S.data){
    if(!tx?.integrationAggregateId)return false;
    tx.integrationNeedsCompletion=false;
    tx.integrationCompletionStatus='completed';
    tx.integrationCompletedAt=nowIso();
    tx.integrationCompletedAs=targetType==='fixa'?'fixa':'variavel';
    tx.integrationCompletionReason='';
    tx.integrationEditGuardHash=editableSnapshotHash(tx);
    updateImportedTargetReference(tx,tx,data);
    return true;
  }
  function transferImportReferenceToFixed(sourceTx,fixed,data=S.data){
    const reference=captureImportReference(sourceTx);
    if(!reference||!fixed)return false;
    Object.assign(fixed,clone(reference),{
      integrationImported:true,integrationManaged:false,integrationImportMode:'native-fixed-from-generic',
      integrationNeedsCompletion:false,integrationCompletionStatus:'completed',integrationCompletedAt:nowIso(),
      integrationCompletedAs:'fixa',integrationCompletionReason:''
    });
    fixed.integrationEditGuardHash=editableSnapshotHash(fixed);
    updateImportedTargetReference(sourceTx,fixed,data);
    return true;
  }
  ...
  function transferImportReference(sourceTx, targetIds, data=S.data){
    ...
    targets.forEach((tx, index) => {
      Object.assign(tx, clone(reference), {integrationDerivedIndex:index + 1, integrationDerivedCount:targets.length,integrationNeedsCompletion:false,integrationCompletionStatus:'completed',integrationCompletedAt:nowIso(),integrationCompletedAs:'variavel',integrationCompletionReason:''});
      tx.integrationEditGuardHash=editableSnapshotHash(tx);
    });
    const first = targets[0];
    if(first) updateImportedTargetReference(sourceTx,first,data);
  ...
  window.BorionInterop = Object.freeze({
    ...
    setExpenseFlowSetting, canTransitionExpense, completeImportedExpense, transferImportReferenceToFixed, removeImportedNativeGroup,
    ...
    __test:{
      ...
      editableSnapshotHash, reverseImportedTransaction,
      beforeImportCutoff, checkAskModeSource, applyConfirmedInstanceRebind,
      mitExpenseMethodKey, normalizeMitExpenseRules, validateMitExpenseRules, commitMitExpenseAuto, makeGenericIntegratedExpense, applyMitFixedExpensePayment, canTransitionExpense, completeImportedExpense, transferImportReferenceToFixed, findImportedNativeItems, removeImportedNativeGroup
    }
  });
})();
