...
(() => {
  ...
  const SCOPES={
    ...
    // V7.6.2 — Patrimônio usa o mesmo editor livre com grade inteligente de 20 px
    // da Visão Geral no computador, preservando layouts separados por tela e dispositivo.
    patrimony_modules:{label:'Módulos do Patrimônio',columns:4,minColumns:2,maxColumns:6,responsive:true,freePlacement:true},
    // V7.9.0 — Estúdio de Gráficos reaproveita o editor livre: cada gráfico do
    // usuário é um módulo independente, com layout próprio por dispositivo.
    charts_studio:{label:'Painel de gráficos',columns:3,minColumns:2,maxColumns:6,responsive:true,freePlacement:true}
  };

  const finiteNumber=value=>Number.isFinite(Number(value));
  const clamp=(value,min,max)=>Math.max(min,Math.min(max,Number(value)||0));
  const FREE_SNAP_STEP=20;
  const snapTo=(value,step=FREE_SNAP_STEP)=>Math.round((Number(value)||0)/step)*step;
  ...
  const ModuleLayout={
    ...
    deviceMode(scope){
      ...
      if(forced||narrow)return 'mobile';
      const tabletWidth=window.matchMedia&&window.matchMedia('(max-width:1366px)').matches;
      const coarsePointer=window.matchMedia&&window.matchMedia('(pointer:coarse)').matches;
      return tabletWidth||coarsePointer?'tablet':'desktop';
    },
    isFreePlacement(scope){
      return Boolean(this.config(scope).freePlacement&&this.deviceMode(scope)==='desktop');
    },
    hasSnapGrid(scope){ return (scope==='overview_dashboard'||scope==='patrimony_modules'||scope==='charts_studio')&&this.isFreePlacement(scope); },
    snapStep(scope){ return this.hasSnapGrid(scope)?FREE_SNAP_STEP:1; },
    snapValue(scope,value){ return this.hasSnapGrid(scope)?snapTo(value,this.snapStep(scope)):Math.round(Number(value)||0); },
    storageKey(scope){
      const mode=this.deviceMode(scope);
      return mode==='default'?scope:`${scope}_${mode}`;
    },
    modeLabel(scope){ const mode=this.deviceMode(scope); return mode==='mobile'?'Celular':(mode==='tablet'?'Tablet':'Computador'); },
    root(create=false){
    ...
    normalize(scope,value){
      ...
      const defaultColumns=mode==='mobile'?1:(mode==='tablet'?4:(cfg.columns||4));
      const minColumns=mode==='mobile'?1:(mode==='tablet'?2:(cfg.minColumns||2));
      const maxColumns=mode==='mobile'?1:(mode==='tablet'?4:(cfg.maxColumns||6));
    ...
    preparePlacement(scope,id){
      ...
      const layout=this.get(scope),hidden=new Set(layout.hidden.map(String));
      const stagingY=this.snapValue(scope,this.storedBottom(scope,layout)+40);
      hidden.delete(key);layout.hidden=Array.from(hidden);
    ...
    activatePlacement(scope,id){
      ...
      const click=event=>{
        ...
        const x=clamp(this.snapValue(scope,event.clientX-rect.left-frame.w/2),0,Math.max(0,rect.width-frame.w));
        const y=Math.max(0,this.snapValue(scope,event.clientY-rect.top-20));
        const pending=this.pendingPlacement;
        this.pendingPlacement=null;
    ...
    toolbarHTML(scope,title){
      ...
      const device=this.deviceMode(scope);
      const columnButtons=device==='mobile'?[1]:(device==='tablet'?[2,3,4]:Array.from({length:(cfg.maxColumns||6)-(cfg.minColumns||2)+1},(_,i)=>(cfg.minColumns||2)+i));
      ...
      const snapGrid=this.hasSnapGrid(scope);
      const activeHelp=free?(snapGrid?`Modo livre · ${mode}. A grade de ${this.snapStep(scope)} px fica visível e posições e tamanhos encaixam automaticamente para facilitar o alinhamento.`:`Modo livre · ${mode}. Mova e redimensione como janelas; módulos podem ficar sobrepostos e nenhum outro bloco será deslocado.`):`Modo de edição · ${mode}. Arraste pelo cabeçalho e redimensione pelo canto.`;
      const snapStatus=snapGrid?`<div class="module-snap-status" title="Posições, larguras e alturas encaixam automaticamente na grade"><span>GRADE</span><b>${this.snapStep(scope)} PX</b><em>ENCAIXE ATIVO</em></div>`:'';
      const layoutMode=free?`${snapStatus}<span class="module-free-badge">POSIÇÃO LIVRE</span>`:`<div class="module-column-picker"><span>COLUNAS</span>${columnButtons.map(n=>`<button type="button" class="${layout.columns===n?'active':''}" onclick="ModuleLayout.setColumns('${scope}',${n})">${n}</button>`).join('')}</div>`;
    ...
    commitFreeFrame(scope,id,frame,{grid=null,slot=null,render=true}={}){
      ...
      const step=this.snapStep(scope);
      const w=Math.max(180,snapTo(Number(frame.w)||item.pxW||180,step));
      item.pxW=Number.isFinite(gridWidth)?Math.min(w,gridWidth):w;
      item.pxX=Math.max(0,snapTo(Number(frame.x)||0,step));
      if(Number.isFinite(gridWidth)) item.pxX=Math.min(item.pxX,Math.max(0,gridWidth-item.pxW));
      item.pxY=Math.max(0,snapTo(Number(frame.y)||0,step));
      if(Number(frame.h)>0){
        item.pxH=Math.max(120,snapTo(Number(frame.h),step));
        item.h=item.pxH;
      }else{
        delete item.pxH;
        delete item.h;
  ...
  document.addEventListener('pointerdown',event=>{
    ...
    if(ModuleLayout.isFreePlacement(scope)){
      ...
      const move=ev=>{
        ...
        nextX=clamp(ModuleLayout.snapValue(scope,startFrame.x+ev.clientX-startX),0,maxX);
        nextY=Math.max(0,ModuleLayout.snapValue(scope,startFrame.y+ev.clientY-startY));
  ...
  document.addEventListener('pointerdown',event=>{
    ...
    if(ModuleLayout.isFreePlacement(scope)){
      if(grid.classList.contains('module-free-bootstrap')) ModuleLayout.captureFreeFrames(scope,grid);
      const pointerId=event.pointerId,id=String(slot.dataset.moduleId),content=slot.querySelector(':scope > .module-layout-content'),readout=slot.querySelector('[data-module-size-readout]');
      const startFrame=ModuleLayout.freeFrame(scope,id,slot);
      if(!startFrame) return;
      ...
      const paint=()=>{
        ...
        if(readout)readout.textContent=`${Math.round(nextW)}×${Math.round(nextH)}`;
        if(content){content.style.height='100%';content.style.overflow='auto';}
        ModuleLayout.updateFreeCanvas(scope,grid);
      };
      const move=ev=>{
        ...
        nextW=clamp(ModuleLayout.snapValue(scope,startFrame.w+ev.clientX-startX),180,maxW);
        nextH=Math.max(120,ModuleLayout.snapValue(scope,(startFrame.h||naturalH)+(ev.clientY-startY)));
        if(!raf) raf=requestAnimationFrame(paint);
      };
  ...
  let resizeTimer=0,lastModes={};
  window.addEventListener('resize',()=>{
    clearTimeout(resizeTimer);
    resizeTimer=setTimeout(()=>{
      // V7.1.3 — antes só observava 'overview_dashboard'; generalizado pra qualquer escopo
      // responsivo (agora inclui 'patrimony_modules') mudar de layout (desktop/tablet/celular)
      // e re-renderizar por completo quando esse é o escopo em edição no momento.
      Object.keys(SCOPES).forEach(scope=>{
        if(!SCOPES[scope].responsive) return;
        const mode=ModuleLayout.deviceMode(scope);
        if(lastModes[scope]&&lastModes[scope]!==mode&&ModuleLayout.activeScope===scope&&typeof renderView==='function') renderView();
        lastModes[scope]=mode;
      });
