/* Borion Finance — V6.46.67 — Compras compartilhadas, valores a receber e reembolsos. */
(function(){
  'use strict';
  ...
  function receiptDestinationLabel(tx){
    if(tx&&tx.reservaBoxId&&S.data.reservas&&Array.isArray(S.data.reservas.boxes)){
      const box=S.data.reservas.boxes.find(item=>item&&String(item.id)===String(tx.reservaBoxId));
      if(box)return 'Reserva: '+(box.nome||'Reserva');
    }
    return tx&&tx.banco?tx.banco:'Conta';
  }
  ...
  function normalizeReceipt(raw){
    ...
    return {
      ...
      banco:raw.banco||'',
      reservaBoxId:raw.reservaBoxId||null,
  ...
  function linkedReceiptTransactions(modelId,parcelId,personId){
    return (S.data.transacoes||[]).filter(tx=>{
      if(!tx||tx.tipo!=='receita'||tx.origem!=='reembolso')return false;
      if(String(tx.compartilhamentoId||'')!==String(modelId||''))return false;
      if(String(tx.compartilhamentoPessoaId||'')!==String(personId||''))return false;
      /* Dados antigos podem não ter gravado o ID da parcela. Quando ele existe,
         exigimos correspondência para impedir que compras diferentes se misturem. */
      return !parcelId||!tx.compartilhamentoParcelaId||String(tx.compartilhamentoParcelaId)===String(parcelId);
    });
  }

  function syncReceipts(parcelOrModel){
    const normalized=normalizeModel(parcelOrModel);
    if(!normalized)return null;
    const parcelId=parcelOrModel&&parcelOrModel.compartilhamento?parcelOrModel.id:(parcelOrModel&&parcelOrModel.compartilhamentoParcelaId)||null;
    normalized.pessoas.forEach(person=>{
      const byTransaction=new Map();
      (person.recebimentos||[]).forEach(receipt=>{
        const tx=paymentTransaction(receipt);
        if(!tx)return;
        byTransaction.set(tx.id,Object.assign({},receipt,{
          ...
          reservaBoxId:tx.reservaBoxId||receipt.reservaBoxId||null
        }));
      });
      /* O lançamento financeiro é a fonte de verdade. Se uma versão anterior criou a
         Receita mas falhou ao anexar o item ao histórico da pessoa, reconstruímos o
         vínculo automaticamente. Assim qualquer valor parcial, múltiplas parcelas ou
         valor maior que a parcela sugerida entra integralmente na progressão. */
      linkedReceiptTransactions(normalized.id,parcelId,person.id).forEach(tx=>{
        if(byTransaction.has(tx.id))return;
        byTransaction.set(tx.id,{
          id:'receipt_'+tx.id,
          transacaoId:tx.id,
          valor:roundMoney(tx.valor),
          data:tx.data||'',
          ...
          banco:tx.banco||'',
          reservaBoxId:tx.reservaBoxId||null,
          createdAt:Number(tx.createdAt)||Date.now()
        });
      });
      person.recebimentos=Array.from(byTransaction.values()).sort((a,b)=>(Number(a.createdAt)||0)-(Number(b.createdAt)||0));
    });
    return normalized;
  }
  ...
  function personReceived(person){
    return roundMoney((person&&person.recebimentos||[]).reduce((sum,r)=>{
      ...
      /* O valor salvo no histórico serve de fallback para dados legados. Não existe
         regra de "múltiplo exato da parcela": todo centavo recebido conta. */
      return sum+(tx?Number(tx.valor)||0:Number(r&&r.valor)||0);
    },0));
  }
  ...
  function summaryHTML(){
    ...
    if(!count)return '';
    return `<div class="shared-receivable-summary"><div><span>Compras compartilhadas</span><b>${count}</b></div><div><span>A receber</span><b>${money(total)}</b></div><div><span>Já reembolsado</span><b>${money(received)}</b></div><p>Esses valores não alteram o total das faturas. Reembolsos recebidos entram na conta, Carteira ou reserva escolhida sem contar como renda.</p></div>`;
  }
  ...
  function registerReceipt(cartaoId,parcelaId,personId){
    ...
    const accountOptions=typeof accountSelectOptions==='function'?accountSelectOptions({}):[];
    const reserveBoxes=(typeof reservasEnabled==='function'&&reservasEnabled()&&S.data.reservas&&Array.isArray(S.data.reservas.boxes))
      ? S.data.reservas.boxes.filter(box=>box&&box.id&&(!box.accountId||typeof accountById!=='function'||accountById(box.accountId)))
      : [];
    const reserveOptions=reserveBoxes.map(box=>({value:box.id,label:(box.nome||'Reserva')+(box.banco?' · '+box.banco:'')}));
    if(!accountOptions.length&&!reserveOptions.length){toast('Cadastre uma conta, Carteira ou reserva para registrar o reembolso.');return;}
    const destinationOptions=[];
    if(accountOptions.length)destinationOptions.push({value:'conta',label:'Conta / Carteira'});
    if(reserveOptions.length)destinationOptions.push({value:'reserva',label:'Reserva'});
    const initialDestination=accountOptions.length?'conta':'reserva';
    openModal({title:'Registrar reembolso',sub:person.pessoa+' deve '+moneyText(pending)+' desta compra. O valor recebido entrará no destino escolhido e não contará como renda.',fields:[
      ...
      {key:'destinationType',label:'Onde o dinheiro entrou',type:'segmented',options:destinationOptions,default:initialDestination},
      {key:'accountId',label:'Conta / Carteira',type:'select',options:accountOptions.length?accountOptions:[{value:'',label:'Nenhuma conta disponível'}],default:(accountOptions[0]||{}).value||'',visibleWhen:{key:'destinationType',value:'conta'}},
      {key:'reservaBoxId',label:'Reserva',type:'select',options:reserveOptions.length?reserveOptions:[{value:'',label:'Nenhuma reserva disponível'}],default:(reserveOptions[0]||{}).value||'',visibleWhen:{key:'destinationType',value:'reserva'}}
      ...
      if(cents(value)>cents(currentPending)){alert('O valor recebido não pode ultrapassar o pendente de '+moneyText(currentPending)+'.');return;}
      const destinationType=values.destinationType==='reserva'?'reserva':'conta';
      const reserveBox=destinationType==='reserva'?reserveBoxes.find(box=>String(box.id)===String(values.reservaBoxId)):null;
      let accountId=null;
      if(destinationType==='reserva'){
        if(!reserveBox){alert('Escolha uma reserva válida.');return;}
        const reserveAccountRef=reserveBox.accountId||(typeof resolveAccountId==='function'?resolveAccountId(reserveBox.banco,{includeArchived:false}):null);
        accountId=typeof requireAccountId==='function'?requireAccountId(reserveAccountRef,'A conta vinculada a esta reserva não está disponível.'):reserveAccountRef;
      }else{
        accountId=typeof requireAccountId==='function'?requireAccountId(values.accountId,'Escolha uma conta válida para receber o reembolso.'):values.accountId;
      }
      if(!accountId)return;
      const bank=typeof accountNameSnapshot==='function'?accountNameSnapshot(accountId):(reserveBox&&reserveBox.banco)||'';
      let tx;
      const ok=typeof runAtomicFinancialMutation==='function'?runAtomicFinancialMutation(()=>{
        tx={id:safeId(),tipo:'receita',nome:'Reembolso — '+person.pessoa+' — '+(parcel.descricao||'Compra compartilhada'),data:values.data||todayISO(),categoria:parcel.categoria||'Outro',valor:value,accountId,banco:bank,origem:'reembolso',reservaValor:0,destinoModo:'Conta livre',compartilhamentoId:model.id,compartilhamentoParcelaId:parcel.id,compartilhamentoPessoaId:person.id,createdAt:Date.now()};
        S.data.transacoes.push(tx);
        if(destinationType==='reserva'){
          if(typeof createLinkedReservaMoveFromTransaction!=='function')throw new Error('reserva_indisponivel');
          tx.destinoModo='Direto para reserva';
          createLinkedReservaMoveFromTransaction(tx,reserveBox,value);
        }
        if(typeof applyTxSaldoEffect==='function'&&!applyTxSaldoEffect(tx))throw new Error('conta_invalida');
        person.recebimentos.push({id:safeId(),transacaoId:tx.id,valor:value,data:tx.data,accountId,banco:bank,reservaBoxId:reserveBox?reserveBox.id:null,createdAt:Date.now()});
        parcel.compartilhamento=model;
      },()=>alert('Não foi possível registrar o reembolso. Nenhum saldo ou reserva foi alterado.')):(()=>{return false;})();
      if(!ok)return;
      saveCurrentData();renderView();openManage(cartaoId,parcelaId);toast(value===currentPending?'Reembolso de '+person.pessoa+' recebido por completo.':'Reembolso parcial de '+person.pessoa+' registrado e somado à progressão.');
    }});
  }
  ...
  function undoReceipt(cartaoId,parcelaId,personId,receiptId){
    ...
    const tx=paymentTransaction(receipt);
    openConfirmModal({title:'Desfazer reembolso?',text:'O recebimento de '+moneyText(tx?tx.valor:receipt.valor)+' de '+person.pessoa+' será removido e o valor voltará a sair do destino onde entrou.',confirmLabel:'Desfazer recebimento',variant:'danger',onConfirm(){
      const ok=runAtomicFinancialMutation(()=>{
        if(tx&&typeof reverseTxSaldoEffect==='function')reverseTxSaldoEffect(tx);
        if(tx&&typeof removeLinkedReservaMoveFromTransaction==='function')removeLinkedReservaMoveFromTransaction(tx);
        if(tx)S.data.transacoes=S.data.transacoes.filter(t=>t.id!==tx.id);
        person.recebimentos=(person.recebimentos||[]).filter(r=>r.id!==receiptId&&r.transacaoId!==(tx&&tx.id));
        parcel.compartilhamento=model;
      },()=>alert('Não foi possível desfazer o recebimento.'));
      if(!ok)return;saveCurrentData();renderView();openManage(cartaoId,parcelaId);toast('Recebimento desfeito.');
    }});
  }
  ...
  function manageHTML(card,parcel,model){
    ...
    const rows=model.pessoas.map(person=>{
      ...
      const history=(person.recebimentos||[]).slice().sort((a,b)=>String(b.data||'').localeCompare(String(a.data||''))).map(receipt=>{
        ...
        return `<div class="shared-receipt-history-row"><span>${escape(tx.data?tx.data.slice(8,10)+'/'+tx.data.slice(5,7)+'/'+tx.data.slice(0,4):'—')} · ${escape(receiptDestinationLabel(tx))}</span><b>${money(tx.valor)}</b><button type="button" onclick="SharedPurchases.undoReceipt('${card.id}','${parcel.id}','${person.id}','${receipt.id}')" title="Desfazer este recebimento">↺</button></div>`;
      }).join('');
