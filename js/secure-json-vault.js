...
(() => {
  ...
  function createContext(options) {
    ...
    // Modo Drive confiável: a conta Google e as permissões da pasta passam a ser
    // a barreira de acesso. Novos JSONs não recebem uma segunda senha. Envelopes
    // antigos continuam abríveis e são convertidos uma única vez, sem perda.
    const driveTrustMode = options?.driveTrustMode === true;
    ...
    let legacyEnvelopeMigrationPending = false;
    ...
    async function requestUnlock(envelope) {
      ...
      for (let attempt = 0; attempt < 3; attempt += 1) {
        const password = await securePrompt({
          ...
          appName,
          theme: dialogTheme,
          title: driveTrustMode ? 'Converter base antiga' : 'Desbloquear dados',
          message: driveTrustMode
            ? 'Este JSON foi criptografado por uma versão anterior. Digite a antiga senha mestra uma última vez para abrir e converter a base. Depois da confirmação no Google Drive, ela não será solicitada novamente.'
            : 'Digite sua senha mestra para abrir a base criptografada.',
          label: driveTrustMode ? 'Senha mestra antiga' : 'Senha mestra',
          placeholder: driveTrustMode ? 'Digite a senha usada anteriormente' : 'Digite sua senha',
          autocomplete: 'current-password',
          submitLabel: driveTrustMode ? 'Abrir e converter' : 'Desbloquear',
          cancelLabel: 'Cancelar',
          error: unlockError
        });
    ...
    async function open(value, options = {}) {
      if (!looksLikeEnvelope(value)) {
        if (!driveTrustMode && isSensitive(value)) {
      ...
      if (driveTrustMode && isSensitive(plain)) legacyEnvelopeMigrationPending = true;
      return plain;
    }

    async function protect(value, options = {}) {
      if (!isSensitive(value)) return value;
      // No modo confiado ao Drive, a proteção é a própria autenticação Google e
      // o escopo drive.file. Mantemos a compatibilidade de leitura com envelopes
      // antigos, mas toda nova gravação é JSON normal e mais rápida.
      if (driveTrustMode) return value;
    ...
    function lock() {
      ...
      template = null;
      plaintextMigrationPending = false;
      legacyEnvelopeMigrationPending = false;
      unlockPromise = null;
      unlockVaultId = '';
    }
    ...
    const context = Object.freeze({
      ...
      isSensitive,
      needsMigration: () => driveTrustMode ? legacyEnvelopeMigrationPending : plaintextMigrationPending,
      markMigrated: () => { plaintextMigrationPending = false; legacyEnvelopeMigrationPending = false; },
      isDriveTrustMode: () => driveTrustMode,
      lock,
      status: () => ({ appId, ownerBound: !!ownerBinding, unlocked: !!key, driveTrustMode, migrationPending: driveTrustMode ? legacyEnvelopeMigrationPending : plaintextMigrationPending, vaultId: template?.vaultId || '', revision: Number(template?.revision || 0) })
    });
